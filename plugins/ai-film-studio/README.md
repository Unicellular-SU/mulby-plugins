...
6. 顶栏「生成全剧」会按剧集顺序逐集执行资产、被分镜引用的形态参考图、关键帧、视频和合成；已成片、失败和暂缓剧集默认跳过，每集成片路径和制作回顾会回写到对应 Episode，供后续集生成时引用。
...
- [ ] 顶栏「生成全剧」能逐集生成资产、分镜实际引用的形态参考图并合成，已成片、失败和暂缓剧集会跳过；多集输出文件名包含集号和集标题，切回对应剧集能看到「已成片」「成片失败」或「已暂缓」状态。
