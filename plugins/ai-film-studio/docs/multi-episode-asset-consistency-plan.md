...
- 资产页已有跨集资产矩阵，分镜页已有一致性详情抽屉；连续性报告会标记同类型角色/场景/道具重名、章节分配、变体适用范围、本集专属形态未绑定和缺失参考图，并提供生成缺失参考图、标记变体适用本集、绑定唯一候选形态、重新均分章节等快速修复。
...
- Episode 切换器已有跨集承接入口，会主动汇总最近制作回顾，以及当前集资产/形态在其他剧集中的出现记录；还会给出补参考图、标记形态适用本集、新建并应用本集形态等建议，可一键顺序处理可执行项。新建本集形态时会基于上一相关剧集状态预填形态提示词。Agent 也可用 `get_episode_handoff` 读取同一套线索和建议。
...
- 系列一致性检查：已支持同类型资产重名检测和本集专属形态未绑定检测，后续继续扩展角色别名、资产复用、变体覆盖深度和场景复用。
...
- 跨集承接建议已支持批量执行可处理项、基于上一相关剧集状态生成新形态提示词并自动回写推荐变体；后续继续扩展更复杂的跨集状态判断。
...
19. 下一步：继续扩展系列一致性质量门，把角色别名、资产复用深度、场景复用和跨集状态变化纳入更自动的检查与 Agent 生成约束。
