...
- **[改用 CapCut/剪映 范式·已完成]** ①放弃 PS 竖排纯图标(隐喻不贴视频)，改 **CapCut/剪映 风格左侧功能面板 `<ToolPanel>`**(w-132，图标+文字瓦片，分组「基础编辑 / 叠加元素 / 输出」，单例工具已存在打小圆点提示"点了是选中")。②**时间轴做成可拖 `<StudioTimeline>`**：播放头 scrub(按下即定位+拖动持续定位)；叠加块可拖(移动/拖左右把手改时间窗，源时间基始终正确，走 updateOpLive→commitLive)；静音块在「源≡输出(无删段无变速)」时可拖、否则只读+提示；字幕 cue 只读(编辑在检查器)。`Block` 组件带左右 ew-resize 把手；`dragBlock` 用 `[data-lane]` 内容区宽算 pxPerSec。typecheck + vite build green。
- **[工具条 PS 化·已被 CapCut 版取代]** 把检查器顶部「添加大类」按钮行（被批不够专业）改为 **PS 风左侧竖排工具条 `<ToolRail>`**（w-14，每工具独立图标 + tooltip + 选中高亮左条）：全局工具(裁切 Scissors/变速 Gauge/几何 Crop/调色 Palette/音频 Music) — 分隔 — 叠加工具(文字 Type/字幕 Captions/水印 Stamp/贴纸 Sticker/打码 Grid2x2/边框 Square/进度条 RectangleHorizontal/时间码 Timer/画中画 PictureInPicture2) — 底部 导出设置 Settings2。点全局=有则选中、无则新增并选中；点叠加=新增并选中。布局变三栏：**工具条 | 预览 | 检查器(图层+参数面板)**。typecheck + vite build green。
