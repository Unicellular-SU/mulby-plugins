...
| P0 | **原子库批量补齐**（NumberStepper / Tooltip / Menu / Popover / Field 族 / Toast / Combobox） | ✅ 已完成（仅构建验证） | 在 Select·Checkbox·Switch 之上补齐设计方案组件库：`NumberStepper`（自绘 ±，抑制 OS 旋钮，文本缓冲容小数/中途编辑；已迁并发字段）、`Tooltip`（Radix + 玻璃气泡，`TooltipProvider` 挂 App 根 delay 400/skip 300）、`Menu`（Radix DropdownMenu，items[] API：icon/danger/checked/shortcut/分隔）、`Popover`（Radix 玻璃面板 + 箭头 + `PopoverClose`）、`Field/Input/Textarea` 族（label/help/error/required + `invalid`→aria-invalid，复用 `.afs-field__input`）、`Toast`（zustand store + 视口挂 App 根 + `toast()` API + 自动消失 + 语义左色条玻璃卡，z-index 10002）、`Combobox`（可搜索下拉：文本触发器 + 前置 Search + 过滤玻璃列表 + 匹配高亮 + 可选「创建」自由值；自带浮层避免与输入焦点冲突，ARIA combobox + aria-activedescendant 键盘导航；对外 value/onChange 与 Select 一致）。Radix 依赖经 **pnpm** 加入。全部 `tsc --noEmit` + `CI=true build:ui` 通过。**注：Toast/Combobox 等暂作基础设施落地**（Toast 现有反馈仍走宿主 `window.mulby.notification`；Combobox 与 Select 可互换，模型选择器迁移属交互变更）；菜单/浮层/Field/Combobox 尚未广泛接到调用点，均留待可视验证再迁。 |
| — | ✅ **里程碑：设计方案计划内原子库全部建成** | ✅ 已完成（仅构建验证） | Select / Checkbox / Switch / NumberStepper / Tooltip / Menu / Popover / Field 族 / Toast / Combobox 共 10 组原子全部落地、令牌化、明暗双主题 + a11y 降级完备、`tsc`+`build` 逐个验证。**仍待**：① 广泛迁移新原子到调用点（菜单/浮层/Field/Combobox）；② 6 屏结构重写；③ in-app Confirm 替换 14 处 `window.confirm`（同步→异步）；④ 动效 keyframes + `.afs-glow` 接 CTA。均建议**有可视验证再做**。 |
| C | **逐屏重设计 · 设置（竖向 Tabs）** — 6 屏结构重写之第 1 屏 | ✅ 已完成（构建 + 4 镜对抗评审；待可视核对） | SettingsView 左导航 → 真·竖向 Tabs：`role=tablist/tab/tabpanel` + 漫游 tabindex + ↑/↓循环·Home/End 键盘（移焦+激活）+ 每项前置 18px 图标（Server/Palette/HardDrive/SlidersHorizontal）+ 选中态渐变左指示条（`--afs-grad-brand`，装饰性）+ `:focus-visible` 环。外观主题卡 → `role=radiogroup/radio` + `aria-checked` + 漫游 tabindex + ←→↑↓ 循环单选（移焦+选中）。清理 2 处硬编码色（`.afs-prov--active` 蓝 / `.afs-prov__del:hover` 红 → `color-mix` 主题感知）+ 补 ProviderSettings textarea/grid 输入缺失的 hover/focus 环。**刻意保留** 14 个原生 `<input>`（由 `.afs-form__row input` 后代 CSS 主题化；换 `<Input>` 改变样式来源、盲改风险高，留待可视后再迁）。**4-agent 对抗评审**（行为/a11y/令牌/规格）：令牌零缺陷、行为 1:1 无回归；修掉 1 高危（单选组只加了 role 却缺漫游 tabindex+方向键 → 补齐）+ 1 低危（非活动 tab 的 `aria-controls` 悬空 → 仅活动项）。`tsc`+`build` 通过。**待用户在真实插件可视核对方向后，再推其余 5 屏（ProjectHome / AssetsView·ResultViewer / Inspector / Studio）。** |
| C | **逐屏重设计 · 其余 5 屏（仅构建验证批）** — ProjectHome / ResultViewer / AssetsView / Inspector / Studio | ✅ 安全子集已完成（构建 + 5-agent understand + 3-镜对抗评审） | 用户授权「仅构建验证」推进。先跑 **5-agent understand 工作流** 出逐屏实现计划 + 行为契约，关键发现：**多数 §6「现状问题」已过时**（原生 select/emoji/硬编码色在早期阶段已清），且大改件（hero / 三栏 / 4 区抽屉 / Tabs·Segmented·Modal 转换）都**缺前置原子（Button/IconButton/Tabs/Segmented/Modal/空状态）+ 需肉眼验证** → 全部 defer。落地的**安全子集**：①ProjectHome 卡片操作收进 **Menu kebab**（同 handler 1:1）+ hover 阴影令牌化；②ResultViewer → **文本玻璃模态**（模糊只在卡片、遮罩保留无模糊 + 全套降级）+ `role=dialog`/aria + 对比度令牌阶梯（去裸 opacity）+ 作用域 chip 覆盖；③AssetsView 合集行渐变左指示条 + 两个关闭按钮补 aria-label；④Inspector 节点标题/参数/覆盖项/片段 **全部包进 Field 原子** + 原生 textarea/text 换 Input/Textarea 原子（number 保留原生——空串强制转换无等价）+ `.afs-result--error` 硬编码红→令牌；⑤Studio 删除**死的 `.afs-studio select{}` 块**（已核验无原生 select 残留）+ `#ef4444`/`#a855f7`→令牌 + Agent 开关渐变 on 态 + 焦点环。每屏 `tsc`+`build` 逐个验证、单独提交。**3-镜对抗评审**（行为/令牌/a11y-cascade）：行为 1:1 零回归、a11y/cascade 干净；逮到 1 中危——`.afs-rv__tile-cap`（媒体上白字）被误指向 `--afs-scrim`（亮色仅 0.30）→ 已还原固定深底。**仍 defer 待可视+补原子**：6 大结构重写（hero/三栏/4 区/各浮层换 Dialog/Tabs/Segmented）、in-app Confirm 替换 14 处 `window.confirm`/`prompt`（同步→异步）、动效 keyframes、Button/IconButton/Tabs/Segmented/Modal/空状态等缺失原子。 |
| P0 | **缺失原子补齐**（解锁结构重写）— Button / IconButton / Segmented / Tabs / Modal+Confirm / EmptyState | ✅ 已完成（构建 + 6-agent 规格抽取 + 2-镜对抗评审；待接线） | 结构重写所缺的 6 个 §5 原子，作为基础设施落地（**暂不接调用点**）：`Button`（薄封装既有 `.afs-btn`：variant secondary/primary/gradient/ghost/danger/danger-solid + size + 图标 + loading + block + glow + pressed，唯一新增 `.afs-btn--block`）、`IconButton`（复用 `.afs-iconbtn` base + 新增 solid/glass/onmedia 变体；必填 aria-label）、`Segmented`（role=radiogroup + 漫游 tabindex + 方向键 + **JS 测量滑动渐变胶囊**）、`Tabs`（Radix react-tabs：横向渐变下划线 + 纵向 rail；TabPanel 再导出）、`Modal`（Radix react-dialog 文本玻璃卡，遮罩无模糊、模糊在卡片，head/body/foot + busy 锁）+ `ConfirmDialog`/`useConfirm()→Promise<boolean>`/`ConfirmProvider`（替代 14 处 `window.confirm`/`prompt` 的异步门，**未挂载/接线**）、`EmptyState`（图标+标题+提示+CTA + 静态极光底）。新增依赖 `@radix-ui/react-{tabs,dialog}`（pnpm）。全部令牌化、明暗双主题、`@supports`/reduced-transparency/reduced-motion/forced-colors 兜底，`tsc`+`build` 通过。**2-镜对抗评审**（逻辑/a11y + CSS/令牌）逮到 2 高 + 1 中 + 3 低，**已全部修复**：①新 `.afs-modal` 与旧版全屏覆盖层 `.afs-modal` 类名冲突（`position:fixed;inset:0` 泄漏）→ 整体改名 `.afs-dialog*`；②Radix Overlay/Content 兄弟节点致遮罩无法居中 → 加独立 `.afs-dialog__center` 居中层（卡片不占 transform，留给缩放动画）；③ConfirmDialog 重叠调用覆盖 resolver 致首个 Promise 永挂 → 新请求前先以 false 结算旧的；④IconButton `Omit<'type'>`；⑤Segmented 活动段恰为 disabled 时漫游落点回退；⑥Segmented 图标-only 段补 aria-label + 图标 aria-hidden。**至此组件库共 16 原子**（10 表单/浮层 + 6 本批）。**下一步（需可视验证）**：把新原子接到调用点 + 6 屏结构重写 + 挂 ConfirmProvider 迁移 14 处确认框 + 动效。 |
| C | **新原子接线进页面**（Segmented / EmptyState 首批落地） | ✅ 已完成（仅构建验证；待可视） | 把新原子接到合适的调用点（行为 1:1，仅换控件外观）：**Segmented** → AssetsView「素材/角色场景库」子页签 + 类型筛选（全部/图片/视频/音频，计数并入标签文本，因 Segmented 无徽标槽）+ 来源筛选（全部来源/生成/上传）+ PromptSettings「本工程/全局默认」作用域页签（均 `value/onChange` 直通原 setState，选项值与联合类型一致）；**EmptyState** → ProjectHome 空、AssetsView 画廊 + 角色场景库空（`loading` 驱动「加载中」）、Inspector「未选中节点」+「未知节点类型」空（图标 + 标题 + 描述 + 静态极光底）。**刻意不接**：Studio 的「暂无」多为卡格内小行内提示（非满屏空态，EmptyState 会过大）、videoMode 是 4 个长标签的窄弹层 Select（改 Segmented 会溢出）、Studio/Dock 既有 bespoke 页签（工作良好、改 Tabs 风险高收益低）、Button/IconButton 与既有 `.afs-btn`/`.afs-iconbtn` 标记几乎等价（纯换皮 churn）。孤立 CSS（`.afs-subtab`/`.afs-scope`/`.afs-lib__empty`/`.afs-chip__n`/`.afs-inspector__empty`）已成死规则但无害。`tsc`+`build` 通过。 |
| C | ✅ **里程碑：6 屏结构重写 + 14 处确认框迁移全部完成**（用户已可视验证基线后授权） | ✅ 已完成（构建 + 7-agent 设计 + 6-agent 实现 + 8-镜对抗评审；待整体可视） | 流程：**设计工作流**（7 并行只读 agent 出逐屏 paste-ready CSS + JSX 方案 + 行为契约）→ 我建 **PromptDialog/usePrompt** + 挂 `ConfirmProvider`/`PromptProvider` 到 App 根 + 迁孤儿确认文件（SettingsView/PromptLibrary/SnapshotPanel）→ **实现工作流**（6 agent 顺序执行，避免共享 styles.css 竞争；各自接线本屏确认框）→ **8-镜评审**（逐屏行为 + CSS 碰撞 + 确认迁移）。**6 屏**：ProjectHome（玻璃工具栏 + Segmented 类型筛 + 搜索过滤渲染子集 + 英雄新建卡）、Inspector（钉头/钉操作区/滚动体/钉删除 4 区 + Button 主次 + glow Run）、Studio（三栏 dock 框架 + Tabs 阶段栏 + ModelBar→Popover 原子 + Dock→Segmented；保守保留设置抽屉/StudioApp/内联标题）、AssetsView（元素编辑器→Modal 原子 + 参考图可选瓦片 aria-pressed + Lightbox 控件→IconButton）、ProviderSettings（14 字段→Field/Section + Input/Textarea/Button/IconButton）、LightboxHost（关闭/导航/连看/发送→IconButton，遮罩保留无模糊）。**14 处 `window.confirm`/`prompt` 全迁到 useConfirm/usePrompt**（含 2 处内联 onClick 提升为具名异步处理器）。评审结论：**零行为回归、零 CSS 跨屏碰撞、零令牌缺失、零 keyframe 冲突**；仅 1 处确认文案漂移（AssetsView 删素材多了「此操作不可撤销。」）已还原。组合 `tsc`+`build` 通过。**遗留（评审记录的保守取舍/低优先）**：Studio 响应式溢出 kebab + 内联标题 Enter/Esc + 设置抽屉换 Dialog（中心锚定会退化右抽屉）+ StudioApp 空态、FilmNode 5 卡体布局重做、PromptLibrary 结构、大量旧 CSS 死规则未清（可逆，留待清理）。**全部仅构建+评审验证，尚未整体可视。** |
| C | ✅ **里程碑：剩余计划项全部做完**（`/loop 把这些全部都做完`，8 次自定步迭代） | ✅ 已完成（仅构建验证；待整体可视） | 逐块推进、每块提交：① **动效基建**——`Skeleton` 原子 + 缺失的 `@keyframes afs-shimmer/afs-breathe`，AssetsView 两处加载态换骨架网格；② **NodeLibrary 搜索框**（过滤 NODE_DEFS）；③ **PromptLibrary 重做**——搜索 + 编辑器浮层换 Modal 原子 + Field/Input/Textarea/Button；④ **FilmNode 收尾**——3 处内联 `style` 锁/看大图按钮 → `.afs-node__lock` 令牌类、运行态状态点加 `.afs-breathe`（节点体逻辑保守不动，画布风险）；⑤ **SearchField 原子**（接 NodeLibrary/PromptLibrary，删重复 CSS）；⑥ **TagsInput 原子**（接 ProviderSettings 音色字段，string[] 保真，弃 join/split）；⑦ **Radio / Slider / InlineEdit 原子**（§5 表单控件库补全，作为基建）；⑧ **死 CSS 清理**——grep 核验后删两大块连续无引用规则（旧 `.afs-lbhost*` ~290 行 + 旧 `.afs-home__*`/`.afs-pcard*` ~112 行 ≈ 400 行），其余与仍用规则交织的死类（`.afs-acard*`/`.afs-prov*`/`.afs-form*`/`.afs-io*` 等）保守留待可视后清。每块 `tsc`+`build` 通过、行为 1:1。**组件库共 23 原子**。**仍待用户整体可视回归。** |
