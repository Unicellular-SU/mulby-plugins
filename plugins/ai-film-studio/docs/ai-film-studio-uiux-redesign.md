...
| P0 | **原子库批量补齐**（NumberStepper / Tooltip / Menu / Popover / Field 族 / Toast） | ✅ 已完成（仅构建验证） | 在 Select·Checkbox·Switch 之上补齐设计方案组件库：`NumberStepper`（自绘 ±，抑制 OS 旋钮，文本缓冲容小数/中途编辑；已迁并发字段）、`Tooltip`（Radix + 玻璃气泡，`TooltipProvider` 挂 App 根 delay 400/skip 300）、`Menu`（Radix DropdownMenu，items[] API：icon/danger/checked/shortcut/分隔）、`Popover`（Radix 玻璃面板 + 箭头 + `PopoverClose`）、`Field/Input/Textarea` 族（label/help/error/required + `invalid`→aria-invalid，复用 `.afs-field__input`）、`Toast`（zustand store + 视口挂 App 根 + `toast()` API + 自动消失 + 语义左色条玻璃卡，z-index 10002）。Radix 依赖经 **pnpm** 加入。全部 `tsc --noEmit` + `CI=true build:ui` 通过。**注：Toast 暂作基础设施落地**（现有反馈仍走宿主 `window.mulby.notification`，迁移属行为变更，留待可视验证）；菜单/浮层/Field 等亦尚未广泛接到调用点。仅剩 **Combobox** 一个计划原子。 |
