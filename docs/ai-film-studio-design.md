...
- toapis 图生视频**仅接受公开图片 URL，不支持 base64**。**已解决**：利用 toapis 的图片上传接口 `POST /v1/uploads/images`（multipart），给 `MediaProviderConfig` 加 `uploadUrl`/`uploadUrlPath`，custom-http 在 submit 前把本地关键帧(data URL)经后端 `uploadImage` RPC（Node `fetch`+`FormData`）上传换公开 URL（`data.url`）再提交 → **t2v 与 i2v 均开箱即用**。已是 http(s) 的图则跳过上传。

> 后续：Sora 官方直连仍需扩展适配器（multipart 提交 + /content 二进制取回）；其它「仅收 URL」供应商也可填 `uploadUrl` 复用此自动上传。
