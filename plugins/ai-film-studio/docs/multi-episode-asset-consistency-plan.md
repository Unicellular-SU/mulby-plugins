...
- 顶栏已有「生成全剧」入口，可按剧集顺序逐集执行资产、被分镜引用的形态参考图、关键帧、视频和合成；已成片剧集默认跳过，每集成片路径写回 `Episode.filmPath`，多集输出文件名包含集号和集标题。
- 每集合成成功后会生成 `Episode.productionRecap`，从本集剧本、分镜、出场资产/变体和视频片段进度提炼制作回顾；Agent 的剧集上下文和工具读取都会暴露该回顾，供后续集承接人物状态和剧情进度。
- Episode 切换器已有重置当前集成片状态入口，可清理 `filmPath/filmError/producedAt/productionRecap`，让该集重新进入「生成全剧」队列。
...
7. 已完成：Episode 切换器可重置当前集成片状态，让已成片/失败剧集重新进入全剧生成队列。
