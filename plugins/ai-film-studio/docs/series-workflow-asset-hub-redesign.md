...
## 第一百三十轮提交：handoff 建议返回使用图谱

本轮继续补 P4/P5 的多集自动修复上下文闭环：

- `handoffSuggestionRef` 新增项目文档和 `usageByEntity` 参数，可为建议项按 `assetId` 注入 `assetCenterUsage`。
- `episodeHandoffView` 和 `episodeHandoffSummary` 的 `suggestions[]` 现在会携带资产中心使用图谱，覆盖 `get_episode_handoff`、`get_workspace`、`get_project_overview`、`get_episodes` 等读入口。
- `apply_episode_handoff_suggestion` 在缺少 suggestionId 时返回的候选建议、执行后的 `applied[]` 和 `remainingSuggestions[]` 也统一保留 `assetCenterUsage`。
- `agentTools.selftest` 增加 workspace handoff 摘要、get_episode_handoff 建议项、apply 结果的使用图谱断言。

验收：

- Agent 读取 handoff 建议时，可以直接从每条建议判断它对应的资产中心身份在当前项目中的集数/形态使用标签。
- 多集自动修复链路中的“读建议 -> 执行建议 -> 读取剩余建议”不再需要在建议项和 planned/shared 资产之间来回反查资产使用图谱。

## 第一百三十一轮提交：身份候选返回使用图谱

本轮继续补 P2/P5 的资产中心身份修复闭环：

- 新增 `identityAssetUsageView`，把 `usageByEntity` 中的身份级使用摘要转换为 Agent 可消费的 `assetCenterUsage`。
- `libraryEntityView` 支持接收项目文档和 usage 图谱，返回身份资产候选、链接成功实体和同步成功实体时统一带出 `assetCenterUsage`。
- `link_project_asset_to_library_entity`、`mark_project_asset_distinct_identity` 与 `sync_project_asset_from_library` 的身份候选错误分支不再返回裸 `libraryEntityView`，会同时保留项目资产 usage 和候选身份 usage。
- `agentTools.selftest` 增加身份链接候选、链接成功实体和同步成功实体的使用图谱断言。

验收：

- Agent 选错或写错资产中心身份名称时，可以直接从 `candidates[]` 判断候选身份是否已经在当前项目、其他项目、画布或快照中使用。
- 多集资产链接/同步/分叉决策不再只依赖项目资产本身的 `assetCenterUsage`，也能读取目标身份资产的全局使用摘要。

## 第一百三十二轮提交：工作区分镜概览返回使用图谱

本轮继续补 P2/P5 的 Agent 首轮读数闭环：

- `overview()` 的 `storyboards[]` 概览新增 `castAssets`，复用正式 `storyboardCastAssets` 轻量视图。
- `get_workspace` 与 `get_project_overview` 因此会在分镜摘要中直接返回项目资产、形态、资产中心 lineage 和 `assetCenterUsage`。
- 该字段不包含图片或长提示词，只补足分镜摘要里原本只有 `castNames` 的结构化资产上下文。
- `agentTools.selftest` 增加 workspace 分镜摘要里 `Hero / Gala` 的 lineage、形态类型和使用图谱断言。

验收：

- Agent 第一轮读取工作区时，可以直接从 `storyboards[].castAssets[]` 判断某个镜头实际用了哪个资产中心身份/形态。
- 多集生产链路中的“看概览 -> 选择镜头继续补图/绑定/修复”不再必须先额外调用 `get_storyboards` 才能拿到镜头资产上下文。

## 第一百三十三轮提交：原著章节分配返回剧集计划使用图谱

本轮继续补 P4/P5 的章节拆分到多集规划闭环：

- `chapterEpisodeRefs` 新增 `usageByEntity` 输入，返回章节所属剧集时保留 `seriesQueueState` 和轻量 `plan`。
- `get_novel` 会加载资产中心使用图谱，并在 `chapters[].episodes[].plan.requiredAssets/requiredVariants` 中带出 `assetCenterUsage`。
- 章节读数仍然不返回完整剧集、分镜或长文本；只补足“该章节分配到哪一集、该集计划使用哪些资产/形态”的上下文。
- `agentTools.selftest` 增加章节 `ch2` 分配到 `ep2` 后，剧集计划资产/形态使用图谱的断言。

验收：

- Agent 读取原著章节时，可以直接从章节所属剧集看到该集计划资产和妆容/形态对应的资产中心身份。
- 多集拆分链路中的“读章节 -> 看已分配剧集 -> 继续规划资产/形态”不再必须额外调用 `get_episodes` 才能补齐计划资产上下文。

## 第一百三十四轮提交：原著搜索命中返回剧集计划使用图谱

本轮继续补 P4/P5 的章节搜索到多集规划闭环：

- `search_project` 在搜索 `novel` 域时也会加载资产中心使用图谱。
- 原著章节搜索命中的 `novel[]` 结果新增 `episodes`，复用 `chapterEpisodeRefs` 返回章节所属剧集、队列状态和轻量计划。
- `novel[].episodes[].plan.requiredAssets/requiredVariants` 与 `get_novel` 保持一致，带出资产中心 `assetCenterUsage`。
- `agentTools.selftest` 增加原著搜索命中章节后，直接读取所属剧集计划资产/形态使用图谱的断言。

验收：

- Agent 通过 `search_project` 命中原著章节时，可以直接知道该章节已分配到哪一集，以及该集计划使用哪些资产中心身份/形态。
- 多集拆分链路中的“搜索章节 -> 继续规划剧集资产/妆容”不再必须先切到 `get_novel` 或 `get_episodes` 补上下文。

## 第一百三十五轮提交：工作区原著概览返回剧集计划使用图谱

本轮继续补 P4/P5 的工作区首轮读数闭环：

- `overview()` 的 `novel[]` 概览新增 `episodes`，复用 `chapterEpisodeRefs` 返回章节所属剧集、队列状态和轻量计划。
- `get_workspace` 与 `get_project_overview` 因此会在章节摘要里直接返回所属剧集的计划资产/形态 `assetCenterUsage`。
- 该字段不返回章节全文或完整剧集，只补足原著章节摘要里原本缺失的“章节 -> 剧集计划 -> 资产中心身份/形态”链路。
- `agentTools.selftest` 增加 workspace 原著章节摘要里 `ch2 -> ep2 -> Hero/Gala` 使用图谱断言。

验收：

- Agent 第一轮读取工作区时，可以直接从 `novel[].episodes[].plan` 判断某个章节已分配剧集的资产中心计划上下文。
- 多集拆分链路中的“看工作区概览 -> 继续规划章节/剧集资产”不再必须额外调用 `get_novel` 或 `get_episodes` 才能补齐章节分配上下文。

## 第一百三十六轮提交：剧本入口返回剧集计划使用图谱

本轮继续补 P4/P5 的剧本读数闭环：

- 新增 `scriptEpisodeContext`，统一为剧本条目返回所属剧集信息和轻量 `episodePlan`。
- `get_workspace` 与 `get_project_overview` 的 `scripts[]` 现在会带出该剧本所属剧集的计划资产/形态 `assetCenterUsage`。
- `search_project` 在搜索 `scripts` 域时也会加载资产中心使用图谱，并在脚本命中结果中返回 `episodePlan`。
- `get_script` 正文读取成功或同集脚本未命中时，会返回同一份所属剧集计划上下文。
- `agentTools.selftest` 增加脚本搜索、工作区脚本摘要和剧本正文读取的计划资产/形态使用图谱断言。

验收：

- Agent 通过剧本搜索或工作区脚本摘要定位到某集剧本时，可以直接判断该集计划要求哪些资产中心身份和妆容/形态。
- 多集剧本续写链路中的“命中剧本 -> 继续补分镜/资产/妆容”不再必须额外调用 `get_episodes` 才能补齐该集计划资产上下文。

## 第一百三十七轮提交：剧本写入口返回剧集计划使用图谱

本轮继续补 P4/P5 的剧本写后回读闭环：

- `upsert_script` 写入或更新某集剧本后，返回的 `episode` 从裸 `episodeInfo` 升级为统一 `scriptEpisodeContext`。
- 写后结果里的 `episode.episodePlan.requiredAssets/requiredVariants` 会携带资产中心 `assetCenterUsage`，与 `get_script`、`search_project.scripts[]` 和工作区脚本摘要保持一致。
- 写入逻辑本身不改变，仍然只切换目标剧集并更新该集剧本；本轮只补充返回上下文。
- `agentTools.selftest` 增加非当前集剧本写入后返回计划资产/形态使用图谱的断言。

验收：

- Agent 写入某集剧本后，可以直接从写入结果判断该集计划资产和妆容/形态对应的资产中心身份。
- 多集剧本生成链路中的“写剧本 -> 继续生成分镜/补资产/处理妆容”不再必须额外调用 `get_script` 或 `get_episodes` 才能拿到该集计划上下文。

## 第一百三十八轮提交：handoff 修复返回剧集计划使用图谱

本轮继续补 P4/P5 的跨集承接修复写后闭环：

- `apply_episode_handoff_suggestion` 执行建议后，返回的 `episode` 在保留 `episodeId/episodeIndex/episodeTitle/current` 兼容字段的同时新增轻量 `plan`。
- `episode.plan.requiredAssets/requiredVariants` 会携带资产中心 `assetCenterUsage`，与 `get_episodes`、`get_episode_handoff` 和计划写入口的使用图谱口径对齐。
- `applied[]` 与 `remainingSuggestions[]` 原本已有使用图谱，本轮补齐的是“执行后目标剧集计划本身”的上下文。
- `agentTools.selftest` 增加自动执行 handoff 建议后返回目标剧集计划资产/形态使用图谱的断言。

验收：

- Agent 自动补齐计划资产主图、形态作用域或形态图后，可以直接从执行结果判断该集计划资产和妆容/形态是否仍指向正确资产中心身份。
- 多集生产链路中的“执行 handoff 修复 -> 继续生成分镜/关键帧/视频”不再必须额外调用 `get_episodes` 才能回读目标集计划上下文。

## 第一百三十九轮提交：新增分镜返回剧集计划使用图谱

本轮继续补 P4/P5 的分镜写后回读闭环：

- `add_storyboard` 新增分镜后，返回的 `episode` 在保留 `episodeId/episodeIndex/episodeTitle/current` 兼容字段的同时新增轻量 `plan`。
- `episode.plan.requiredAssets/requiredVariants` 会携带资产中心 `assetCenterUsage`，与工作区概览、剧集列表、剧本写入口和 handoff 修复结果的口径对齐。
- 分镜本身的 `storyboard.castAssets` 原本已带使用图谱，本轮补齐的是“新增分镜后目标剧集计划本身”的上下文。
- `agentTools.selftest` 增加新增非当前集分镜后返回计划资产/形态使用图谱的断言。

验收：

- Agent 新增某集分镜后，可以直接从写入结果确认该集计划资产和妆容/形态对应的资产中心身份。
- 多集生产链路中的“写分镜 -> 继续补计划资产/绑定形态/生成关键帧或视频”不再必须额外调用 `get_episodes` 才能回读目标集计划上下文。

## 第一百四十轮提交：关键帧与视频生成返回剧集计划使用图谱

本轮继续补 P4/P5 的生产生成写后回读闭环：

- 新增 `episodeInfoWithPlan`，统一为写工具返回目标剧集兼容字段和轻量 `plan`。
- `apply_episode_handoff_suggestion` 与 `add_storyboard` 的既有计划返回改用该 helper，避免后续入口继续手写计划视图。
- `generate_keyframe` 和 `generate_clip` 生成成功后，返回的 `episode.plan.requiredAssets/requiredVariants` 会携带资产中心 `assetCenterUsage`。
- 生成结果里的 `storyboard.castAssets` 原本已带使用图谱，本轮补齐的是“生成后目标剧集计划本身”的上下文。
- `agentTools.selftest` 增加关键帧生成和视频片段生成后返回计划资产/形态使用图谱的断言。

验收：

- Agent 生成关键帧或视频片段后，可以直接从结果确认目标集计划资产和妆容/形态仍指向正确资产中心身份。
- 多集生产链路中的“生成关键帧/视频 -> 判断是否继续补计划资产/形态/承接修复”不再必须额外调用 `get_episodes` 才能回读目标集计划上下文。

## 第一百四十一轮提交：分镜编辑返回剧集计划使用图谱

本轮继续补 P4/P5 的分镜修复写后回读闭环：

- `set_storyboard_cast_variant`、`set_storyboard_asset_ref` 和 `set_storyboard_scene_asset` 成功写入后，返回的 `episode` 改用统一 `episodeInfoWithPlan`。
- 这些分镜编辑工具的 `episode.plan.requiredAssets/requiredVariants` 会携带可用的资产中心 `assetCenterUsage`；已分叉资产继续按既有规则不误挂使用图谱。
- 分镜本身的 `storyboard.castAssets` 或 `storyboards[].castAssets` 原本已带使用图谱，本轮补齐的是“编辑后目标剧集计划本身”的上下文。
- `agentTools.selftest` 增加形态绑定、资产引用和场景资产统一后的剧集计划上下文断言。

验收：

- Agent 修复某集分镜里的角色形态、补资产引用或统一场景资产后，可以直接从结果确认目标集计划资产/形态上下文。
- 多集修复链路中的“改分镜引用 -> 判断是否还需要补计划、作用域或 handoff 修复”不再必须额外调用 `get_episodes` 才能回读目标集计划上下文。

## 第一百四十二轮提交：剧集计划写入口返回统一计划上下文

本轮继续补 P4/P5 的计划写后回读一致性：

- `upsert_episode_plan` 写入或更新单集计划后，返回的 `episode` 从裸 `episodeInfo` 改为统一 `episodeInfoWithPlan`。
- 顶层 `plan` 字段保持不变，继续作为直接写入结果；新增的是 `episode.plan` 也携带同一组计划资产/形态和资产中心 `assetCenterUsage`。
- 这样计划写入口、剧本写入口、分镜写入口、分镜编辑入口和生成入口都能用同一套 `episode.plan` 结构继续执行后续动作。
- `agentTools.selftest` 增加剧集计划写后 `episode.plan.requiredAssets/requiredVariants` 使用图谱断言。

验收：

- Agent 写入某集计划后，可以直接从 `episode.plan` 判断目标集计划资产和妆容/形态对应的资产中心身份。
- 多集规划链路中的“写计划 -> 继续写剧本/分镜/执行 handoff 修复”不再依赖顶层 `plan` 的特殊返回形态，也不需要额外调用 `get_episodes` 补上下文。

## 第一百四十三轮提交：形态作用域候选返回剧集计划使用图谱

本轮继续补 P4/P5 的写入纠错闭环：

- `set_asset_variant_scope` 在 `scopeKind=episode` 但无法解析目标剧集时，候选 `episodes` 从裸 `episodeInfo` 改为统一 `episodeListWithUsage`。
- 候选剧集现在会携带轻量 `plan.requiredAssets/requiredVariants`，并保留对应资产中心 `assetCenterUsage`。
- 成功写入/移除形态作用域的逻辑不变，本轮只补齐错误候选返回体，方便 Agent 在选错 episode scope 后直接重试。
- `agentTools.selftest` 增加无效 episode scope 候选剧集里 `Hero/Gala` 计划使用图谱断言。

验收：

- Agent 为妆容/服装/时期形态设置 episode 适用范围时，如果给错剧集序号，可以直接从错误返回里看到各集计划资产和形态上下文。
- 多集修复链路中的“设置形态作用域失败 -> 选择正确剧集重试”不再必须额外调用 `get_episodes` 才能判断目标集。

## 第一百四十四轮提交：跨集承接入口返回统一剧集计划上下文

本轮继续补 P4/P5 的跨集承接读数一致性：

- `get_episode_handoff` 成功读取某集承接线索时，顶层剧集信息从裸 `episodeInfo` 改为统一 `episodeInfoWithPlan`。
- 返回体保留原有 `plannedAssets/plannedVariants/sharedAssets/suggestions`，同时新增 `plan.requiredAssets/requiredVariants` 及其资产中心 `assetCenterUsage`。
- 这样 handoff 入口与 `get_episodes`、计划写入口、剧本/分镜写入口的 `episode.plan` 口径一致。
- `agentTools.selftest` 增加 `get_episode_handoff.plan` 中 `Hero/Gala` 使用图谱断言。

验收：

- Agent 读取某集 handoff 后，可以直接用统一的 `plan` 字段判断目标集计划资产和妆容/形态上下文。
- 多集生产链路中的“读 handoff -> 修复计划/形态作用域 -> 生成分镜或视频”不再需要在 handoff 专用字段和 episode plan 字段之间做特殊分支。

## 第一百四十五轮提交：分镜读入口返回统一剧集计划上下文

本轮继续补 P4/P5 的分镜读数一致性：

- `get_storyboards`、`get_storyboard_table` 和 `get_timeline` 的顶层剧集信息从裸 `episodeInfo` 改为统一 `episodeInfoWithPlan`。
- 这些入口原本已经返回分镜、设计表或时间线里的资产中心使用图谱，本轮补齐同一返回体里的 `plan.requiredAssets/requiredVariants`。
- `get_storyboards` 现在即使只分页读取分镜，也会在顶层返回目标集计划资产/形态上下文，方便后续继续写分镜或生成关键帧。
- `agentTools.selftest` 增加三类分镜读入口 `plan` 中 `Hero/Gala` 使用图谱断言。

验收：

- Agent 读取某集分镜、设计分镜表或时间线后，可以直接从同一返回体判断目标集计划资产和妆容/形态上下文。
- 多集生产链路中的“读分镜/大纲/时间线 -> 修复计划/绑定形态/生成画面或视频”不再必须额外调用 `get_episodes` 才能补齐目标集计划上下文。

## 第一百四十六轮提交：分镜搜索命中返回剧集计划使用图谱

本轮继续补 P4/P5 的搜索入口上下文一致性：

- `search_project` 的 `storyboards` 命中从裸 `episodeInfo` 改为 `episodeInfoWithPlan`，命中分镜会直接带目标集 `plan`。
- `search_project` 的 `storyboardTable` 命中同样返回 `plan.requiredAssets/requiredVariants` 和资产中心 `assetCenterUsage`。
- 分镜命中的 `castAssets` 与设计表命中的 `resolvedCastAssets/resolvedAssetRefs` 保持不变，本轮只补齐同一搜索结果里的剧集计划上下文。
- `agentTools.selftest` 增加分镜搜索与分镜表搜索命中的 `Hero/Gala` 计划使用图谱断言。

验收：

- Agent 通过搜索定位到某集分镜或设计表场次后，可以直接判断该集计划资产和妆容/形态上下文。
- 多集生产链路中的“搜索分镜/场次 -> 修复资产/绑定形态/生成镜头”不再必须额外调用 `get_storyboards`、`get_storyboard_table` 或 `get_episodes` 才能补齐目标集计划上下文。

## 第一百四十七轮提交：工作区分镜摘要返回剧集计划使用图谱

本轮继续补 P4/P5 的工作区首屏读数闭环：

- `overview()` 的 `storyboards[]` 摘要从裸 `episodeInfo` 改为 `episodeInfoWithPlan`。
- `get_workspace` 与 `get_project_overview` 因此会在分镜摘要里直接返回所属剧集 `plan.requiredAssets/requiredVariants`。
- 分镜摘要里原有 `castAssets` 资产中心使用图谱保持不变，本轮补齐的是“分镜 -> 所属剧集计划 -> 资产中心身份/形态”的同屏上下文。
- `agentTools.selftest` 增加工作区分镜摘要中 `Hero/Gala` 计划使用图谱断言。

验收：

- Agent 第一轮读取工作区时，可以直接从 `storyboards[]` 判断该分镜所属剧集的计划资产和妆容/形态上下文。
- 多集生产链路中的“看工作区概览 -> 继续修复某集分镜/生成镜头”不再必须额外调用 `get_episodes` 才能补齐目标集计划上下文。

## 第一百四十八轮提交：系列规划面板展示资产中心使用图谱

本轮从 Agent 读数闭环转向人工规划面板的可见性：

- `SeriesTab` 的必需项目资产选项接入 `assetHubStore.usageByEntity`，在资产名旁展示资产中心使用 chip。
- 必需形态/妆容选项复用父项目资产的资产中心图谱，帮助用户在勾选多集妆容时确认它属于哪个全局身份。
- 使用图谱只读展示，不改变 `Episode.plan.requiredAssetIds/requiredVariantIds` 的持久化结构和写入逻辑。
- `styles.css` 补充紧凑 chip 样式，保证长资产名和使用标签在规划面板内截断而不撑破布局。

验收：

- 用户在“系列 -> 剧集规划”里勾选必需资产或形态时，可以直接看到该选项是否已有资产中心项目/资产/画布/快照使用记录。
- 多集规划中的“同一人不同妆容”不再只能靠名称判断，人工规划时能同步看到身份使用图谱，降低误选同名资产或错误形态的风险。

## 第一百四十九轮提交：系列规划形态选项提示缺图与作用域

本轮继续完善人工规划面板的多集资产一致性提示：

- `SeriesTab` 的必需形态/妆容选项保留 `AssetVariant.refImageId` 与 `appliesToEpisodeIds` 元数据。
- 每集渲染时按当前 `episode.id` 计算就绪提示：形态缺少参考图时显示“缺形态图”，形态作用域已限定但不包含当前集时显示“未适用本集”。
- 提示只作为 UI 警告 chip 和 hover title 展示，不改变 `Episode.plan.requiredVariantIds`、`AssetVariant.appliesToEpisodeIds` 或任何持久化结构。
- 资产中心使用 chip 继续保留，形成“身份来源 + 形态就绪 + 剧集作用域”的同屏判断。

验收：

- 用户在“系列 -> 剧集规划”勾选某集必需妆容时，可以直接看到该妆容是否已经有形态图，以及是否标记适用于当前集。
- 多集生产中的“同一人不同妆容”不再只依赖名称和人工记忆，能在规划阶段发现缺图或作用域错配，减少后续分镜、关键帧和视频生成时的资产引用错误。

## 第一百五十轮提交：系列规划项目资产提示缺主参考图

本轮继续补齐人工规划面板的生产就绪提示：

- `SeriesTab` 的必需项目资产选项保留 `Asset.refImageId` 就绪状态。
- 角色、场景、道具缺少主参考图时，选项旁显示“缺主参考图”警告 chip，并在 hover title 中同步说明。
- 该提示复用上一轮的规划 warning 样式，与资产中心使用 chip 同屏展示，不改变 `Episode.plan.requiredAssetIds` 或资产生成链路。
- 这样系列规划面板能同时暴露“父资产主图是否就绪”和“形态/妆容参考图、作用域是否就绪”。

验收：

- 用户在“系列 -> 剧集规划”勾选某集必需角色、场景或道具时，可以直接看到该项目资产是否已经有主参考图。
- 多集规划阶段可以提前发现缺父资产主图的问题，避免后续 handoff、分镜关键帧或视频生成前才暴露生产输入不完整。

## 第一百五十一轮提交：系列规划形态勾选自动维护父资产

本轮继续把人工规划入口与 Agent 计划写入规则对齐：

- `SeriesTab` 的必需形态/妆容选项保留父项目资产 `assetId`。
- 用户勾选某个形态/妆容时，会自动把该形态所属父资产并入当前集 `requiredAssetIds`，与 `upsert_episode_plan` 的写入规则一致。
- 用户取消某个父资产时，会同步移除该父资产下已经勾选的必需形态，避免人工 UI 产生“只规划形态、未规划父资产”的不一致状态。
- 对旧项目里已经存在的错配计划，形态选项会显示“未规划父资产”警告 chip，帮助用户在系列规划面板直接发现并修正。

验收：

- 用户在“系列 -> 剧集规划”只勾选某个妆容/服装/状态时，父角色、场景或道具会自动进入本集必需项目资产。
- 人工取消父资产不会留下孤立的必需形态，减少后续连续性报告、handoff 修复和生产生成前的计划错配。

## 第一百五十二轮提交：系列规划剧集卡展示生产输入摘要

本轮继续提升多集规划面板的扫描效率：

- `SeriesTab` 为每集卡片头部新增计划摘要 chip，显示本集已规划的必需项目资产和必需形态总数。
- 摘要会统计缺主参考图、缺形态图、形态未适用本集、未规划父资产和无效计划引用，并以“风险 N”在卡片头部提示。
- hover 摘要可查看计划资产数、计划形态数和风险分类，用户不必逐个阅读所有勾选项才能发现本集生产输入问题。
- 该摘要只读取现有 `Episode.plan`、`Asset.refImageId` 与 `AssetVariant` 作用域字段，不改变任何持久化结构或生成链路。

验收：

- 用户浏览“系列 -> 剧集规划”时，可以快速看到每集是否未规划、已规划且就绪，或存在缺图/作用域/父资产/无效引用风险。
- 多集项目的规划阶段能先按剧集定位问题，再进入具体资产或形态选项修正，降低长剧集列表中的漏检成本。

## 第一百五十三轮提交：系列规划标题栏展示整季就绪汇总

本轮把上一轮的单集摘要提升为整季级总览：

- `SeriesTab` 将每集生产输入 readiness 计算收敛为局部 `readinessForEpisode`，避免卡片摘要和整季汇总重复维护两套统计口径。
- “剧集规划”标题栏新增整季汇总 chip，显示缺少剧集、未规划剧集、存在风险剧集和已就绪剧集数量。
- 汇总 hover 可查看已建剧集、计划集数和各类规划状态，适合几十集短剧先判断整季推进状态。
- 本轮仍只读取 `Episode.plan`、项目资产和形态作用域，不改变持久化结构，也不自动生成或修复资产。

验收：

- 用户进入“系列 -> 剧集规划”后，不需要滚动查看每张卡，也能先看到整季还有多少集未创建、未规划、存在生产输入风险或已经就绪。
- 多集规划的检查顺序从“逐集排查”升级为“整季汇总定位 -> 单集卡片详情 -> 具体资产/形态修正”，更适合长短剧项目。

## 第一百五十四轮提交：系列规划支持按就绪状态筛选剧集

本轮继续完善长剧集列表的定位效率：

- `SeriesTab` 新增本地 `SeriesPlanFilter` 状态，支持按“全部 / 未规划 / 风险 / 就绪”筛选剧集卡片。
- 筛选按钮复用整季 readiness 统计口径，按钮上直接显示对应剧集数量，避免汇总数字和实际列表口径不一致。
- 当前筛选下没有剧集时显示轻量空状态，不改变任何 `Episode.plan` 数据。
- 该筛选只影响系列规划面板的可视列表，用户仍可通过“全部”回到完整剧集视图。

验收：

- 用户看到整季汇总里有风险或未规划剧集后，可以一键筛到对应剧集集合，再逐个修复资产、形态或作用域问题。
- 多集项目从“看汇总但仍要手动找卡片”进一步收敛到“汇总 -> 筛选 -> 修复”的工作流。

## 第一百五十五轮提交：系列规划整季汇总可直接触发筛选与补集

本轮把整季汇总从只读提示升级为操作入口：

- `SeriesTab` 标题栏里的整季汇总 chip 改为按钮。
- 点击“未规划 / 风险 / 就绪”汇总项会直接切换到对应筛选视图，与下方筛选按钮保持同一套 `SeriesPlanFilter` 状态。
- 点击“缺集”会执行现有 `fillEpisodes()` 补齐缺少剧集，并自动切换到“未规划”视图，方便用户继续补计划资产和形态。
- 样式从静态 `i` 标签收敛为 button 态，保留 warning/ready 颜色和当前筛选高亮。

验收：

- 用户看到整季汇总里的风险或未规划数量时，可以直接点汇总项定位到对应剧集。
- 当计划集数大于已建剧集数时，用户可以从同一个汇总区域补齐缺集并立即进入未规划剧集处理流。

## 第一百五十六轮提交：系列规划支持复用上一集生产输入

本轮继续降低多集资产一致性的人工维护成本：

- `SeriesTab` 为 E2 之后的剧集卡片头部新增“复用上一集生产输入”图标按钮。
- 该动作只复制上一集 `Episode.plan.requiredAssetIds` 和 `requiredVariantIds`，不覆盖本集的开场钩子、冲突、结尾钩子等叙事字段。
- 当上一集没有计划资产或形态时，按钮保持禁用，并在 hover 提示上一集尚无生产输入。
- 复制后仍走现有 `updateEpisodePlan` 清洗逻辑，继续过滤无效资产/形态引用，并保留形态作用域、缺图、父资产等 readiness 提示。
- 样式采用 22px 图标按钮，放在剧集标题和摘要 chip 之间，避免长标题或状态标签撑破卡片布局。

验收：

- 用户在规划连续多集短剧时，可以先为 E1 设置基础角色、场景、道具和妆容，再逐集复用到后续集并只调整差异项。
- 同一角色跨集保持同一个项目资产和形态引用，不需要每集从完整资产列表重新勾选，减少误选同名资产或漏选妆容的风险。

## 第一百五十七轮提交：系列规划支持批量沿用上一集输入

本轮把上一轮的单集复用动作扩展为整季批量入口：

- `SeriesTab` 标题栏新增“沿用上集 N”按钮，只在存在可补齐的未规划剧集时显示。
- 批量动作按剧集顺序扫描：遇到已有生产输入的剧集就作为后续来源，遇到未规划剧集则复制最近上一集的 `requiredAssetIds` 和 `requiredVariantIds`。
- 批量复制仍然不覆盖每集的开场钩子、冲突、结尾钩子等叙事字段，只补生产输入矩阵。
- 连续多个未规划剧集会沿用同一组最近生产输入，适合先规划 E1 的基础角色/场景/道具，再把基础输入铺到 E2...En。
- 复制后仍交给现有 `updateEpisodePlan` 清洗无效引用，并由 readiness 摘要继续暴露缺图、作用域或父资产风险。

验收：

- 用户为前一集规划好基础生产输入后，可以一键把这些输入批量铺到后续未规划剧集，再逐集增删差异资产或妆容。
- 多集项目不再需要对几十集逐个点“复用上一集”，但仍不会误改每集独立的故事钩子和冲突内容。

## 第一百五十八轮提交：形态卡支持按剧集计划同步适用范围

本轮把“系列规划”与“项目资产形态卡”的作用域维护打通：

- `AssetVariantCard` 新增计划使用摘要，直接显示哪些剧集的 `Episode.plan.requiredVariantIds` 要求了当前形态/妆容。
- 当计划中已有使用集数时，形态卡显示“按计划适用”动作，可把 `AssetVariant.appliesToEpisodeIds` 同步为这些剧集。
- 如果当前形态已经是“全剧适用且计划覆盖全剧”，或显式适用范围已经与计划集数一致，该动作会禁用，避免重复写入。
- 该动作只更新当前形态的适用范围，不改 `Episode.plan`，不生成或替换参考图；后续仍由现有 readiness 与连续性报告提示缺图、父资产或分镜绑定问题。
- 计划摘要使用紧凑 chip 展示，长剧集列表只显示前 5 个计划集数并折叠剩余数量，避免形态卡撑宽。

验收：

- 用户先在“系列 -> 剧集规划”勾选某个妆容/服装用于 E3/E4 后，回到项目资产形态卡可以一键把该形态作用域同步为 E3/E4。
- “同一个人不同妆容”的维护不再只能从形态卡逐集手点作用域，剧集计划成为作用域同步的来源，降低多集妆容错配风险。

## 第一百五十九轮提交：跨集资产矩阵展示剧集计划输入

本轮继续把项目资产页升级为“系列资产矩阵”：

- `AssetContinuityPanel` 的矩阵行新增“计划剧集”和“计划形态”两组 chip，直接读取 `Episode.plan.requiredAssetIds/requiredVariantIds`。
- 只在剧集计划中出现、但还没有进入分镜实际出场的资产也会进入矩阵，避免计划资产在项目资产页不可见。
- 计划剧集既统计直接要求父资产的剧集，也统计要求该资产下任一形态/妆容的剧集；旧项目里只规划形态未规划父资产的情况也能被看到。
- 实际出场集数继续来自连续性报告的 `castUses`，计划集数来自 `Episode.plan`，两者并排展示，方便用户判断“计划了但还没用”或“用了但不在计划里”。
- 资产中心图谱和问题数量保留在同一行，矩阵标题栏增加“进入剧集计划”的资产数量摘要。

验收：

- 用户在“系列 -> 剧集规划”里把某个角色、场景、道具或妆容纳入 E2/E3 后，即使分镜还没绑定，该资产也会出现在项目资产页的跨集资产矩阵。
- 用户可以在同一行对照计划剧集、实际出场剧集、实际形态、计划形态和资产中心图谱，降低多集资产漏用或妆容错配的排查成本。

## 第一百六十轮提交：跨集资产矩阵标出计划与出场差异

本轮让系列资产矩阵从“并排展示”进一步进入差异排查：

- `AssetContinuityPanel` 现在会对比每个资产的计划剧集与实际分镜出场剧集。
- 如果某资产已经进入 `Episode.plan`，但对应剧集还没有实际分镜出场，矩阵状态列会显示“计划未用 N”。
- 如果项目已经开始维护剧集计划，但某资产实际出场于未规划该资产的剧集，矩阵状态列会显示“未计划 N”。
- 差异 chip 的 hover title 会列出具体剧集，便于用户回到系列规划或分镜绑定处修正。
- 未做任何剧集计划的旧项目不会因为历史分镜出场被批量标为“未计划”，避免迁移期噪音。
- 原有连续性问题 chip 继续保留在同一状态列，与计划/出场差异共同构成资产矩阵的排查入口。

验收：

- 用户能直接在项目资产页看到“计划了但还没用于分镜”的资产，及时补分镜绑定或调整剧集计划。
- 用户也能发现“已经在某集分镜出场但不在该集计划中”的资产，降低多集生产中临时引用绕过系列资产圣经的风险。

## 第一百六十一轮提交：跨集资产矩阵标出形态计划差异

本轮把上一轮的计划/出场差异细化到妆容、服装、状态等形态层：

- `AssetContinuityPanel` 现在会按 `episodeId + variantId` 对比计划形态与实际分镜形态。
- 如果某集计划要求某个 `AssetVariant`，但该集分镜没有实际使用这个形态，矩阵状态列显示“形态未用 N”。
- 如果项目已经开始维护剧集计划，但某集分镜实际使用的形态没有进入该集 `requiredVariantIds`，矩阵状态列显示“形态未计划 N”。
- 差异 hover title 会列出 `E集数/形态名称`，方便用户直接定位是哪一集的哪种妆容或服装错配。
- 未维护任何剧集计划的旧项目继续不显示“形态未计划”，避免迁移期噪音。
- 父资产级的“计划未用 / 未计划”与形态级的“形态未用 / 形态未计划”共同保留，覆盖“资产没用上”和“资产用了但妆容不对”两类问题。

验收：

- 用户在 E3 计划了“晚宴妆”但分镜仍使用主形象或其他妆容时，可以直接在资产矩阵里看到形态未用。
- 用户在 E4 分镜临时绑定了某个妆容但没有回写剧集计划时，可以直接看到形态未计划，避免后续生成链路绕过系列资产圣经。

## 第一百六十二轮提交：跨集资产矩阵支持差异筛选

本轮把系列资产矩阵从“展示所有行”推进到“按问题定位”：

- `AssetContinuityPanel` 新增本地 `AssetMatrixFilter`，支持“全部 / 计划差异 / 连续性问题”三种视图。
- 筛选按钮直接显示当前矩阵内对应资产行数量，复用上一轮的资产级与形态级计划差异统计。
- “计划差异”视图会同时收纳“计划未用、未计划、形态未用、形态未计划”四类行，方便长剧集项目快速定位规划与实际分镜不一致的资产。
- “连续性问题”视图只显示已有 `buildContinuityReport` 问题的资产行，保留与详情抽屉质量门一致的排查入口。
- 有计划差异的行也会使用 warning 边框，不再只有连续性 issue 行被高亮。
- 当前筛选下无行时显示轻量空状态，不改变项目资产、剧集计划或连续性报告数据。

验收：

- 用户在几十集项目里看到矩阵标题提示存在计划/出场差异后，可以直接点“计划差异”只看需要修正的资产。
- 用户需要处理连续性报告问题时，可以点“连续性问题”只看有质量门 issue 的资产，不被纯展示型资产行干扰。

## 第一百六十三轮提交：跨集资产矩阵支持未入图谱筛选

本轮继续把项目资产页作为“项目快照与资产中心身份边界”的核对入口：

- `AssetMatrixFilter` 新增 `unlinked` 视图，在资产中心 Hub 已加载后显示“未入图谱”筛选。
- `AssetContinuityPanel` 会统计当前矩阵里没有 `assetCenterUsage` chip 的项目资产，并在标题栏显示“未入图谱 N”。
- 点击“未入图谱”后只显示尚未进入资产中心使用图谱的项目资产，方便用户优先处理缺少全局身份链接、快照或画布采纳证据的角色、场景和道具。
- 该筛选只读取现有 `assetCenterChips` 与 Hub 加载状态，不改变项目资产、身份资产或剧集计划。
- Hub 尚未加载时不显示该筛选，避免把加载中的资产误判为未入图谱。

验收：

- 用户在项目资产页可以直接找出尚未绑定到资产中心身份或缺少图谱证据的项目资产。
- 多集项目在进入生产前，可以先过滤“未入图谱”资产，逐个决定发布、链接、分叉或保持项目内专属资产。

## 第一百六十四轮提交：跨集资产矩阵标出未入图谱行

本轮把上一轮的“未入图谱”筛选补齐为行级状态：

- `AssetContinuityPanel` 的矩阵行高亮条件加入 `rowMissingAssetCenter`，没有资产中心使用图谱的项目资产会和计划差异、连续性问题一样使用 warning 边框。
- 状态列新增“未入图谱”chip，hover title 指明该项目资产尚未进入资产中心图谱。
- 该状态只在 Hub 已加载且当前行没有 `assetCenterChips` 时出现，继续避免把加载中的资产误判为缺失图谱。
- 资产中心列原有“未入图谱”占位继续保留；新增状态 chip 让用户在横向扫描时不需要专门阅读图谱列也能发现身份边界问题。
- 本轮仍只读取现有 Hub usage，不自动发布、链接、分叉或同步任何资产。

验收：

- 用户切到“全部”视图时，也能通过 warning 边框和“未入图谱”状态快速发现还没进入资产中心图谱的项目资产。
- 用户切到“未入图谱”视图时，每一行都有明确状态 chip，方便逐个决定该资产是否应该发布、链接或保持项目专属。

## 第一百六十五轮提交：跨集资产矩阵折叠列提供完整摘要

本轮继续提升长剧集项目里的矩阵可读性：

- 新增 `assetMatrixChipsetTitle`，统一为矩阵 chip 列生成完整 hover 摘要。
- “出现剧集”“计划剧集”“使用形态”“计划形态”四列现在都会在列级 `title` 中展示完整列表。
- 当某列折叠成 `+N` 时，用户不需要重新切换筛选或查看其他面板，也能直接 hover 看到全部集数或全部形态名称。
- 空状态同样保留明确摘要，例如“未出场”“未计划”“未绑定形态”“未计划形态”。
- 本轮只增强现有矩阵展示，不改变剧集计划、分镜绑定、资产中心图谱或连续性报告数据。

验收：

- 用户在几十集短剧里看到 `+N` 折叠 chip 时，可以直接 hover 对应列确认完整的计划/实际集数。
- 同一人物有多个妆容、服装或状态形态时，用户可以直接 hover “使用形态/计划形态”列核对完整形态列表，减少误判。

## 第一百六十六轮提交：跨集资产矩阵按处理优先级排序

本轮让系列资产矩阵在长列表里更符合排查顺序：

- `AssetContinuityPanel` 新增行级优先级计算：连续性问题优先，其次是计划/出场差异、未入资产中心图谱、已进入剧集计划、已实际出场、普通角色行。
- 同一优先级内继续按问题数量、差异项数量、计划集数、出场集数、资产名称和资产 id 做稳定排序。
- 所有筛选视图复用同一排序结果，用户切到“计划差异”“连续性问题”或“未入图谱”时也会先看到更需要处理的资产。
- 该排序只影响项目资产页矩阵展示，不改变资产列表顺序、剧集计划、分镜引用或资产中心图谱。

验收：

- 用户打开“全部”视图时，不需要先滚动查找，连续性问题、计划差异和未入图谱资产会自然排在前面。
- 用户在任一筛选视图中处理多条资产时，问题更多或影响更多集数的资产会更靠前，方便逐项修复。

## 第一百六十七轮提交：跨集资产矩阵支持已规划筛选

本轮补齐系列资产矩阵的计划视图入口：

- `AssetMatrixFilter` 新增 `planned`，筛选出已经进入 `Episode.plan` 的项目资产行。
- “已规划”数量复用现有 `plannedAssetCount`，与标题栏里的“进入剧集计划”统计保持同一口径。
- 该视图包含直接规划父资产的行，也包含只通过计划形态关联到父资产的行，延续矩阵对“形态已规划但父资产未直接规划”旧数据的可见性。
- “已规划”视图继续复用矩阵优先级排序，计划内但有连续性问题、计划差异或未入图谱的资产会靠前。
- 本轮只增加前端筛选入口，不改变 `Episode.plan`、项目资产顺序、分镜引用或资产中心图谱。

验收：

- 用户在“系列 -> 剧集规划”维护完生产输入后，可以切到项目资产页的“已规划”视图，集中核对本季计划资产和形态。
- 长剧集项目里，用户不必在“全部”视图中混着未计划角色和临时资产查找计划内资产。

## 第一百六十八轮提交：跨集资产矩阵支持已出场筛选

本轮补齐系列资产矩阵的实际出场视图入口：

- `AssetMatrixFilter` 新增 `appeared`，筛选出已经进入分镜 `castUses` 的项目资产行。
- 矩阵标题栏新增“已有分镜出场”数量摘要，口径与“出现剧集”列一致。
- 筛选按钮新增“已出场”，与上一轮“已规划”形成计划输入和实际生产绑定的两端入口。
- “已出场”视图继续复用矩阵优先级排序，实际出场但存在连续性问题、计划差异或未入图谱的资产会靠前。
- 本轮只增加前端筛选和摘要，不改变 `Episode.plan`、分镜 `castRefs`、项目资产或资产中心图谱。

验收：

- 用户可以在项目资产页直接切到“已出场”视图，集中核对已经被分镜实际使用的角色、场景和道具。
- 多集项目排查“计划外临时引用”时，可以先看“已出场”资产，再结合“计划差异”状态定位哪些资产已经绕过剧集计划进入生产链路。

## 第一百六十九轮提交：跨集资产矩阵支持待落分镜筛选

本轮把“计划差异”里最常见的生产前缺口拆成独立入口：

- `AssetMatrixFilter` 新增 `unused`，筛选已经进入剧集计划但尚未进入分镜实际绑定的项目资产行。
- 行级判断复用现有 `plannedUnusedLabels` 和 `plannedVariantUnusedLabels`，同时覆盖“计划资产未出场”和“计划形态未绑定”两类待落分镜问题。
- 矩阵标题栏新增“计划待落分镜”warning 摘要，让用户不切筛选也能看到待补分镜绑定的资产规模。
- 筛选按钮显示为“待落分镜”，与“已规划 / 已出场 / 计划差异”形成更清晰的生产检查顺序：先看计划输入，再补落分镜，再核对实际出场和其他差异。
- 本轮只增加前端筛选和摘要，不修改 `Episode.plan`、分镜 `castRefs`、项目资产、形态作用域或资产中心图谱。

验收：

- 用户在系列规划完成后，可以直接切到“待落分镜”视图，集中处理计划了但还没写入分镜的角色、场景、道具或妆容。
- 多集项目进入关键帧/视频生成前，可以先清空“待落分镜”视图，降低计划资产漏绑定或计划妆容未使用的风险。

## 第一百七十轮提交：跨集资产矩阵支持计划外出场筛选

本轮补齐“待落分镜”的对偶排查入口：

- `AssetMatrixFilter` 新增 `unplanned`，筛选已经进入分镜实际绑定、但没有进入对应剧集计划的项目资产行。
- 行级判断复用现有 `unplannedUseLabels` 和 `unplannedVariantUseLabels`，同时覆盖“资产计划外出场”和“形态计划外使用”两类问题。
- 矩阵标题栏新增“计划外出场”warning 摘要，让用户打开项目资产页时能直接看到临时分镜引用是否绕过了系列规划。
- 筛选按钮显示为“计划外”，与“待落分镜”一起把计划/实际差异拆成两个明确动作方向：一个是把计划补进分镜，一个是把分镜使用回写计划或替换为正确资产/形态。
- 旧项目未维护任何剧集计划时，仍沿用现有 `hasAnyEpisodePlan` 保护，不会把历史分镜批量标为计划外。
- 本轮只增加前端筛选和摘要，不修改 `Episode.plan`、分镜 `castRefs`、项目资产、形态作用域或资产中心图谱。

验收：

- 用户可以切到“计划外”视图，集中核对已经出现在分镜里但没有进入剧集计划的角色、场景、道具或妆容。
- 多集项目在生成关键帧/视频前，可以先处理计划外出场，避免临时资产或错误妆容绕过系列资产圣经进入生产链路。

## 第一百七十一轮提交：跨集资产矩阵支持形态差异筛选

本轮把同一人物不同妆容、服装、状态的错配从综合计划差异中拆出来：

- `AssetMatrixFilter` 新增 `variant`，筛选存在计划形态未绑定或分镜形态未进入计划的项目资产行。
- 行级判断复用现有 `plannedVariantUnusedLabels` 和 `unplannedVariantUseLabels`，不重新计算分镜或剧集计划数据。
- 矩阵标题栏新增“形态差异”warning 摘要，让用户打开项目资产页时能直接看到妆容/服装/状态层面的不一致规模。
- 筛选按钮显示为“形态差异”，与“待落分镜”和“计划外”并列，但只聚焦形态层，不混入只有父资产计划/出场差异的行。
- 本轮只增加前端筛选和摘要，不修改 `Episode.plan`、分镜 `castRefs`、项目资产、形态作用域或资产中心图谱。

验收：

- 用户可以切到“形态差异”视图，集中核对 E3 战损妆、E5 晚宴妆这类计划妆容是否真正绑定到分镜。
- 多集项目处理同一人物不同形态时，不必在所有计划差异里手动分辨资产级缺口和妆容/状态错配。

## 第一百七十二轮提交：跨集资产矩阵筛选按工作意图分组

本轮收敛系列资产矩阵筛选的信息架构：

- 矩阵筛选从单排平铺改为“范围 / 差异 / 质量”三组。
- “范围”包含 `全部 / 已规划 / 已出场`，用于切换资产集合。
- “差异”包含 `待落分镜 / 计划外 / 形态差异 / 计划差异`，用于处理计划与实际分镜不一致。
- “质量”包含 `连续性问题`，在 Hub 已加载时继续包含 `未入图谱`。
- 分组只改变前端展示结构，所有筛选 id、计数口径、排序规则和空状态保持不变。
- 样式增加轻量分隔和组名，不引入新的卡片层级，避免项目资产页在长剧集项目里继续变重。

验收：

- 用户进入跨集资产矩阵时，可以先按“范围”确认计划/实际资产集合，再按“差异”处理待落分镜、计划外和形态错配，最后按“质量”处理连续性问题和资产中心图谱问题。
- 筛选项继续复用原有按钮交互，已有工作流不会因为分组调整改变数据或筛选结果。

## 第一百七十三轮提交：跨集资产矩阵显示列头

本轮继续把项目资产页里的跨集资产矩阵做成可扫读的生产面板：

- 在筛选区和资产行之间新增列头，标明 `资产 / 出现剧集 / 计划剧集 / 使用形态 / 计划形态 / 资产中心 / 状态`。
- 列头复用资产行同一套 grid 轨道，保证标题和内容列对齐。
- 窄屏布局下隐藏列头，避免两列折叠布局里出现误导性的桌面列名。
- 本轮只增加前端可读性提示，不改变矩阵行数据、筛选、排序、hover 摘要、状态 chip 或资产中心图谱计算。

验收：

- 用户第一次打开跨集资产矩阵时，不需要依赖 hover 或记忆也能理解每一列代表计划、实际、形态还是资产中心图谱。
- 长剧集项目里，用户扫描多行资产时可以按列比较计划剧集、出现剧集和形态差异，降低误读成本。

## 第一百七十四轮提交：跨集资产矩阵状态列补正常态

本轮补齐矩阵状态列的空白语义：

- `AssetContinuityPanel` 新增行级 `rowHasStatusWarning` 判断，统一复用连续性问题、计划差异和未入图谱三类状态来源。
- 状态列改为每行始终渲染；没有 warning/issue 的行显示低干扰的“正常”chip。
- “正常”chip 的 hover 提示说明该资产当前没有计划差异、连续性问题或资产中心图谱警告。
- 现有 warning 行高亮、状态 chip、筛选计数和排序规则保持不变。
- 本轮只补前端状态可读性，不改变连续性报告、剧集计划、分镜绑定、形态作用域或资产中心图谱计算。

验收：

- 用户按列扫描矩阵时，状态列不会再出现空白单元，能明确区分“没有问题”和“有问题但没显示”。
- 长剧集项目处理完计划差异、形态差异和图谱问题后，可以直观看到剩余行进入正常态。

## 第一百七十五轮提交：跨集资产矩阵补齐筛选结果反馈

本轮继续把系列资产矩阵做成可操作的生产检查面板：

- 非“全部”筛选时，矩阵标题栏显示“当前显示 X/Y”，让用户知道当前视图命中了多少资产。
- 空筛选状态会显示当前筛选名称，例如“当前没有符合「计划外」筛选的资产”，避免把“已清零”和“筛选语义不明确”混在一起。
- 空筛选状态提供“显示全部”入口，用户可以直接回到完整矩阵，不需要再回到筛选按钮组里查找“全部”。
- 本轮只增强筛选反馈与导航，不修改矩阵筛选口径、排序、状态计算、剧集计划、分镜引用或资产中心图谱。

验收：

- 用户处理完“待落分镜 / 计划外 / 形态差异 / 未入图谱”等视图后，看到空状态即可判断该类问题当前已经清零。
- 长剧集项目里反复切换筛选时，用户可以明确知道当前视图覆盖多少资产，并能从空状态一键回到全量资产矩阵。

## 第一百七十六轮提交：跨集资产矩阵支持按资产类型筛选

本轮继续提升长剧集项目里的矩阵排查效率：

- `AssetContinuityPanel` 新增独立的类型筛选状态，支持 `全部类型 / 人物 / 场景 / 物品`。
- 类型筛选可以和现有 `范围 / 差异 / 质量` 筛选组合，例如只看“人物 + 形态差异”或“道具 + 未入图谱”。
- 现有范围、差异、质量筛选按钮的计数会按当前类型范围重新计算，让用户知道该类型下还有多少计划缺口、计划外出场、形态差异或连续性问题。
- 空状态会显示组合筛选名称，例如“人物 / 计划外”，并且“显示全部”会同时清空类型筛选和状态筛选。
- 本轮只增强矩阵前端筛选维度，不修改资产数据、剧集计划、分镜引用、连续性报告、排序优先级或资产中心图谱。

验收：

- 长剧集项目里，用户可以先限定人物、场景或道具，再处理该类型下的计划差异和图谱问题，避免在混合资产列表里反复扫描。
- 同一个人的多妆容排查可以直接从“人物 + 形态差异”进入，减少场景、道具行对妆容/状态核对的干扰。

## 第一百七十七轮提交：跨集资产矩阵支持名称与别名搜索

本轮继续增强长剧集资产矩阵的定位能力：

- `AssetContinuityPanel` 新增资产搜索输入，支持按项目资产 `name` 和 `aliases` 匹配。
- 搜索会先作用于整张矩阵，类型筛选和 `范围 / 差异 / 质量` 筛选再在搜索结果内继续收窄。
- 类型筛选和状态筛选的计数会跟随搜索上下文变化，让用户搜索某个角色昵称后能直接看到该命中范围内还有多少计划差异、形态差异或图谱问题。
- 空状态会显示组合条件，例如“搜索‘阿瑶’ / 人物 / 形态差异”；“显示全部”会同时清空搜索、类型筛选和状态筛选。
- 本轮只增强矩阵前端定位，不修改资产别名数据、Agent 匹配逻辑、剧集计划、分镜引用、连续性报告或资产中心图谱。

验收：

- 长剧集项目里，用户可以通过角色昵称、称谓或原著别称快速定位同一项目资产，而不是在所有人物/场景/道具行里滚动查找。
- 多妆容排查时，用户可以先搜索某个角色，再结合“人物 + 形态差异”确认该角色在各集的妆容/状态计划是否落到分镜。

## 第一百七十八轮提交：跨集资产矩阵显示别名提示

本轮补齐上一轮搜索能力的可解释性：

- 资产矩阵的资产列在项目资产存在 `aliases` 时显示低干扰的“别名 N”提示。
- 资产名称的 hover title 会列出完整别名，用户搜索昵称、称谓或原著别称后，可以确认命中来自哪个别名。
- 别名提示只显示数量，不把所有别名展开到行内，避免长剧集资产矩阵被别名文本撑宽。
- 本轮只增强矩阵行展示，不修改资产别名数据、搜索匹配逻辑、剧集计划、分镜引用、连续性报告或资产中心图谱。

验收：

- 用户通过别名搜索到资产后，不需要打开资产卡片也能知道该行确实存在别名，并可悬停查看完整别名列表。
- 多集项目处理同一角色的昵称、称谓和不同剧集称呼时，资产矩阵能更清楚地表达“这是同一项目资产的别名”，减少误建新角色的风险。

## 第一百七十九轮提交：跨集资产矩阵统一组合筛选计数

本轮修正上一轮组合筛选后的计数口径：

- `AssetContinuityPanel` 的“范围 / 全部”计数改为当前搜索与类型筛选范围内的资产数，而不是始终显示矩阵全量资产数。
- “质量 / 连续性问题”计数改为当前搜索与类型筛选范围内存在连续性问题的资产行数。
- 计划、出场、差异、未入图谱等计数原本已经按当前类型范围计算，本轮让“全部”和“连续性问题”与它们保持同一口径。
- 本轮只修正前端筛选按钮计数，不修改搜索匹配、筛选结果、排序、连续性报告、剧集计划或资产中心图谱。

验收：

- 用户搜索某个角色或切到“人物 / 场景 / 物品”后，筛选按钮上的数量与当前可筛范围一致，不再混入全量资产数量。
- 长剧集项目里组合使用搜索、类型和质量筛选时，用户可以直接信任按钮计数来判断当前范围内还有多少问题行。

## 第一百八十轮提交：跨集资产矩阵支持按剧集筛选

本轮继续把系列资产矩阵从全局列表推进到单集排查入口：

- `AssetContinuityPanel` 新增“剧集”下拉筛选，支持 `全部剧集` 和每个具体 E 集。
- 剧集筛选会在名称/别名搜索之后执行，类型筛选和 `范围 / 差异 / 质量` 筛选再在当前剧集范围内继续收窄。
- 单集匹配口径覆盖该集实际出场、该集剧集计划、该集连续性问题，以及没有具体剧集归属的全局资产问题。
- 剧集下拉选项显示当前搜索上下文下每集涉及的资产行数量，帮助用户快速定位哪一集需要处理。
- “显示全部”会同时清空搜索、剧集筛选、类型筛选和状态筛选。
- 本轮只增强矩阵前端定位，不修改剧集计划、分镜引用、连续性报告、排序、资产数据或资产中心图谱。

验收：

- 多集项目里，用户可以直接切到 E3/E5 等具体剧集，只查看该集计划或实际涉及的角色、场景、道具和妆容状态。
- 同一个人的不同妆容排查可以从“剧集 + 人物 + 形态差异”进入，减少跨集全量矩阵中的干扰行。

## 第一百八十一轮提交：跨集资产矩阵按剧集收窄问题状态

本轮修正剧集筛选后的问题显示口径：

- `AssetContinuityPanel` 新增当前剧集问题匹配逻辑，单集筛选时只把该集、前序对比到该集、或没有具体剧集归属的全局问题计入当前视图。
- 标题栏“问题”数量改为当前搜索、剧集、类型组合范围内的可见问题数量。
- “质量 / 连续性问题”筛选改为只匹配当前剧集相关问题行。
- 状态列的问题 chip 改为显示当前剧集相关问题数量和 hover 摘要，避免 E3 视图里显示只属于 E5 的问题。
- 本轮只修正矩阵前端问题显示与筛选口径，不修改连续性报告、问题生成逻辑、剧集计划、分镜引用或资产中心图谱。

验收：

- 用户切到某一集后，矩阵里的“连续性问题”筛选和状态 chip 只反映该集相关问题或全局身份问题。
- 多集项目逐集排查时，用户不会因为同一资产在其他剧集的问题而误判当前集仍有未处理的连续性问题。

## 第一百八十二轮提交：跨集资产矩阵按剧集收窄计划差异状态

本轮继续修正单集排查视图中的差异口径：

- `AssetContinuityPanel` 的矩阵行新增带 `episodeId` 的出现、计划、形态和计划差异条目，原有标签仍保留为兼容展示数据。
- 当用户选择某一集时，“已规划 / 已出场 / 待落分镜 / 计划外 / 形态差异 / 计划差异”的计数和筛选只基于当前剧集可见条目。
- 矩阵排序的连续性问题数量、计划差异数量、计划剧集和出场剧集比较改为使用当前剧集可见条目，避免其他剧集的问题把当前集行提前。
- 行内“出现剧集 / 计划剧集 / 使用形态 / 计划形态”列以及状态列中的计划差异 chip 改为显示当前剧集范围内的标签。
- 本轮只调整资产矩阵前端视图口径，不修改 `Episode.plan`、分镜 `castRefs`、连续性报告生成、资产中心图谱或项目持久化结构。

验收：

- 用户切到 E3 时，`待落分镜 / 计划外 / 形态差异 / 计划差异` 不再显示或统计只属于 E5 的差异。
- 单集排查视图里的状态 chip、筛选计数和行内计划/出场/形态列使用同一个剧集范围，减少多集资产矩阵的跨集干扰。

## 第一百八十三轮提交：跨集资产矩阵剧集计数跟随类型范围

本轮继续收敛资产矩阵组合筛选的反馈口径：

- `AssetContinuityPanel` 的剧集下拉计数改为基于当前名称/别名搜索和当前资产类型筛选计算。
- 用户先切到“人物 / 场景 / 物品”后，剧集选项里的数量不再混入其他资产类型。
- 当状态筛选不是“全部”时，标题栏“当前显示 X/Y”的分母改为当前搜索、剧集、类型范围内的资产数量，帮助用户判断当前状态视图还剩多少待处理行。
- 没有状态筛选时，“当前显示 X/Y”继续以矩阵全量资产作为分母，保留搜索、剧集和类型收窄后的全局占比感。
- 本轮只修正资产矩阵前端计数和范围反馈，不修改剧集计划、分镜引用、连续性报告、资产中心图谱或项目持久化结构。

验收：

- 用户选择“人物”后再打开剧集下拉，E3/E5 的数量只代表该搜索范围内的人物资产行。
- 用户进入“E3 / 人物 / 形态差异”时，“当前显示 X/Y”的分母代表 E3 人物范围，而不是整季所有资产行。

## 第一百八十四轮提交：跨集资产矩阵类型计数跟随状态范围

本轮继续修正组合筛选下的按钮计数：

- `AssetContinuityPanel` 抽出 `rowMatchesAssetMatrixFilter`，统一复用 `范围 / 差异 / 质量` 状态筛选判断。
- “类型”筛选按钮的数量改为基于当前名称/别名搜索、当前剧集和当前状态筛选计算。
- 用户先选择“形态差异 / 计划外 / 连续性问题”等状态后，`人物 / 场景 / 物品` 按钮上的数量会反映该状态范围内的可见行。
- 点击某个类型按钮后的结果数量不再和按钮计数脱节，减少长剧集项目里反复试点空类型的成本。
- 本轮只调整资产矩阵前端筛选计数和 helper 复用，不修改剧集计划、分镜引用、连续性报告、资产中心图谱或项目持久化结构。

验收：

- 用户进入“E3 / 形态差异”后，类型按钮显示的是 E3 中存在形态差异的人物、场景、物品数量。
- 如果某类型在当前状态下没有问题，按钮数量显示为 0，点击后不会和上一层计数产生认知冲突。

## 第一百八十五轮提交：跨集资产矩阵剧集计数跟随状态范围

本轮继续把剧集下拉计数与组合筛选结果对齐：

- `AssetContinuityPanel` 的可见标签、可见问题和状态筛选 helper 增加可选剧集参数。
- 当前矩阵行渲染仍使用当前选中的剧集；剧集下拉计数改为用每个候选剧集分别计算当前状态筛选命中数。
- 当用户先选择“形态差异 / 计划外 / 连续性问题”等状态后，E3/E5 等剧集选项的数量会反映该状态在对应剧集中的命中行。
- `全部剧集` 选项也会基于当前搜索、类型和状态范围统计，而不是只显示类型范围内的全量资产。
- 本轮只调整资产矩阵前端筛选计数和 helper 参数化，不修改剧集计划、分镜引用、连续性报告、资产中心图谱或项目持久化结构。

验收：

- 用户选择“形态差异”后打开剧集下拉，E3 的数量只代表 E3 中存在形态差异的资产行。
- 用户选择“连续性问题”后，剧集下拉可以直接看出哪些剧集仍有该类问题，减少逐集试错切换。

## 第一百八十六轮提交：跨集资产矩阵显示当前筛选上下文

本轮继续提升长剧集排查时的可读性：

- `AssetContinuityPanel` 在存在搜索、剧集、类型或状态筛选时，标题栏新增“筛选：...”上下文标签。
- 该标签复用空状态中的 `activeFilterLabel`，让非空结果视图也能直接看到当前组合条件。
- 长搜索词或长剧集标题会在头部截断并通过 hover title 查看完整内容，避免撑乱矩阵筛选区。
- “当前显示 X/Y”继续保留，负责表达数量；新增筛选标签负责表达当前结果属于哪个范围。
- 本轮只增强资产矩阵前端可读性，不修改筛选结果、剧集计划、分镜引用、连续性报告、资产中心图谱或项目持久化结构。

验收：

- 用户进入“搜索角色名 / E3 / 人物 / 形态差异”后，不需要等空状态出现，也能在矩阵头部看到当前组合筛选上下文。
- 长剧集标题或长搜索词不会把头部布局撑宽，完整内容可通过 hover 查看。

## 第一百八十七轮提交：跨集资产矩阵支持非空状态清空筛选

本轮继续完善资产矩阵的组合筛选退出路径：

- `AssetContinuityPanel` 新增 `resetAssetMatrixFilters`，统一清空名称/别名搜索、剧集筛选、类型筛选和状态筛选。
- 非空筛选结果视图的标题栏新增“清空”按钮，用户不需要逐个回退搜索、剧集、类型和状态筛选。
- 空状态中的“显示全部”复用同一个清空 helper，避免非空和空结果的筛选重置逻辑分叉。
- 新增按钮采用低干扰小尺寸样式和 `X` 图标，不改变现有筛选按钮分组和矩阵行布局。
- 本轮只增强资产矩阵前端导航，不修改筛选结果、剧集计划、分镜引用、连续性报告、资产中心图谱或项目持久化结构。

验收：

- 用户在“E3 / 人物 / 形态差异”等非空视图下，可以从矩阵头部一键回到完整跨集资产矩阵。
- 空结果和非空结果使用同一套清空逻辑，后续维护筛选条件时不会出现两个入口行为不一致。

## 第一百八十八轮提交：跨集资产矩阵支持定位项目资产卡

本轮继续把系列资产矩阵从只读排查面板推进到可操作入口：

- `AssetContinuityPanel` 新增矩阵行级“定位到项目资产卡”按钮，用户可以从问题行直接滚动到对应资产卡。
- `AssetCard` 根节点新增稳定 DOM id 和可聚焦状态，定位后会把目标资产卡置于视口中部并显示焦点轮廓。
- 资产矩阵名称列复用已有 `Link2` 图标，小尺寸按钮不改变矩阵列结构、筛选结果或状态 chip 排布。
- 本轮只增强项目资产页内部导航，不修改资产数据、剧集计划、分镜引用、连续性报告、资产中心图谱或项目持久化结构。

验收：

- 用户在“待落分镜 / 计划外 / 形态差异 / 未入图谱”等矩阵视图发现问题后，可以一键跳到对应项目资产卡继续生成、发布、同步或维护形态。
- 被定位的资产卡有明确焦点反馈，长剧集项目里不需要手动在多组人物、场景、物品卡片中查找同名资产。

## 第一百八十九轮提交：跨集资产矩阵显示身份资产同步状态

本轮继续把“项目资产页升级为系列资产矩阵”的资产中心绑定信息前置：

- `AssetContinuityPanel` 复用 `projectAssetLinkStatusLabels`，在矩阵“资产中心”列显示项目资产与身份资产的关系状态。
- 状态 chip 覆盖 `快照 / 已关联 / 有新版 / 已归档 / 已分叉 / 旧链接` 等既有口径，和资产卡上的身份状态保持一致。
- “质量”筛选组新增“身份状态”，只筛出 `有新版 / 已归档 / 已分叉 / 旧链接` 这类需要用户确认的行，避免正常快照资产被误当作问题。
- 标题栏新增“身份状态待确认”摘要，矩阵排序也会把这类行排在普通计划/出场资产之前。
- 本轮只增强矩阵前端同步状态可见性，不修改项目资产、身份资产、同步逻辑、连续性报告、剧集计划或项目持久化结构。

验收：

- 用户不打开单个资产卡，也能在跨集资产矩阵里看到某个项目资产是否来自快照、是否有资产中心新版、是否已分叉或仍是旧链接。
- 多集项目处理全局身份更新时，可以先切到“身份状态”视图，集中确认需要同步、恢复、分叉解释或迁移旧链接的资产。

## 第一百九十轮提交：跨集资产矩阵支持同步身份资产新版

本轮继续把资产矩阵从排查入口推进到可处理入口：

- `AssetContinuityPanel` 在存在资产中心新版的项目资产行上显示“同步”按钮。
- 同步动作复用现有 `syncAssetFromLibraryEntity`，与连续性详情抽屉中的“同步资产中心新版”保持同一条写入路径。
- 只有非分叉、未归档、且 `libraryLink.entityVersion` 落后于资产中心实体版本的行才显示同步按钮。
- 同步成功后沿用现有通知文案，项目资产仍保持快照生产语义，不会直接依赖全局可变身份对象。
- 本轮只增加矩阵行级操作入口，不修改同步算法、资产中心实体、剧集计划、分镜引用、连续性报告或项目持久化结构。

验收：

- 用户切到“身份状态”视图后，可以直接在有新版的资产行上同步项目快照，不必再打开资产卡或连续性详情抽屉。
- 已分叉或已归档身份不会在矩阵中暴露同步按钮，避免绕过既有的分叉边界和归档保护。

## 第一百九十一轮提交：跨集资产矩阵支持发布未入图谱资产

本轮继续补齐系列资产矩阵里的资产中心闭环：

- `AssetContinuityPanel` 在“未入图谱”项目资产行上新增“发布”按钮。
- 发布动作复用现有 `promoteAssetToElement`，成功后刷新 `assetHubStore`，与资产卡上的“发布到资产中心”保持同一条写入路径。
- 只有已有主参考图、未分叉、且关联身份未归档的行才显示发布按钮，避免无图资产或分叉身份绕过既有保护。
- 发布仍保持项目资产快照生产语义，只把当前项目资产显式发布为资产中心身份，不会让项目生产链路直接依赖全局可变对象。
- 本轮只增加矩阵行级发布入口，不修改发布算法、资产中心实体模型、剧集计划、分镜引用、连续性报告或项目持久化结构。

验收：

- 用户切到“未入图谱”视图后，可以直接从矩阵把有主图的项目人物、场景或物品发布到资产中心。
- 没有主参考图、已分叉或归档身份相关的资产不会显示发布按钮，仍需先按既有流程补图、解释分叉或恢复身份。

## 第一百九十二轮提交：跨集资产矩阵支持补主参考图

本轮继续打通“未入图谱 -> 补图 -> 发布”的生产闭环：

- `AssetContinuityPanel` 在缺少主参考图、且已经进入计划或实际出场的项目资产行上新增“生成图”按钮。
- 生成动作复用现有 `generateAsset`，与资产卡上的“生成图片”保持同一条生成路径。
- 正在生成中的资产不会重复显示生成按钮，而是显示“生成中”状态。
- 该入口解决资产矩阵里发现“未入图谱但无法发布，因为缺主图”的断点，让用户可以先补图再发布到资产中心。
- 本轮只增加矩阵行级生成入口，不修改资产生成算法、提示词润色、资产中心发布、剧集计划、分镜引用或项目持久化结构。

验收：

- 用户在跨集资产矩阵中看到已规划或已出场但缺主参考图的资产时，可以直接从矩阵触发主图生成。
- 生成中的资产不会重复触发生成请求，矩阵会用状态 chip 表达当前正在补图。

## 第一百九十三轮提交：跨集资产矩阵支持筛出缺主图资产

本轮继续把“补图 -> 发布”闭环前置到可排查入口：

- `AssetContinuityPanel` 新增“缺主图”质量筛选，命中已进入剧集计划或已经在分镜出场、但还没有主参考图的项目资产。
- 矩阵头部新增缺主图数量摘要，数量仍跟随当前搜索、剧集和类型筛选范围，便于长剧项目按单集集中补图。
- 缺主图资产会进入行级 warning 和排序优先级，不再在没有计划差异、连续性问题或资产中心问题时被误显示为“正常”。
- 行状态新增“缺主图”chip，生成中的资产继续显示“生成中”，避免同一个资产同时暴露缺口和重复生成入口。
- 本轮只补齐矩阵筛选、统计和状态可见性，不修改资产生成算法、资产中心发布、剧集计划、分镜引用、连续性报告或项目持久化结构。

验收：

- 用户可以在跨集资产矩阵中直接切到“缺主图”，批量定位已规划或已出场但无法稳定生成/发布的资产。
- 缺主图资产会明确显示 warning 状态和“生成图”入口，不会被误归为正常行。

## 第一百九十四轮提交：跨集资产矩阵支持补形态参考图

本轮把同一人物不同妆容、服装、伤痕等形态的补图入口也前置到矩阵：

- `AssetContinuityPanel` 新增“缺形态图”质量筛选，命中已进入剧集计划或已被分镜引用、但对应 `AssetVariant.refImageId` 缺失的项目资产行。
- 矩阵头部新增缺形态图数量摘要，继续跟随当前搜索、剧集和类型筛选范围，便于按单集集中处理妆容/状态图缺口。
- 缺形态图资产会进入行级 warning 和排序优先级，不再只依赖连续性详情抽屉或单个资产卡暴露问题。
- 行状态新增“缺形态图”chip，并在父资产已有主参考图时显示“生成形态”按钮，生成动作复用现有 `generateAssetVariant`。
- 正在生成中的形态显示“形态生成中”，不会重复暴露同一形态的生成入口；父资产缺主图时仍需先走主图补齐流程。
- 本轮只补齐矩阵筛选、统计和行级形态生成入口，不修改形态生成算法、作用域规则、资产中心发布、剧集计划、分镜引用、连续性报告或项目持久化结构。

验收：

- 用户可以在跨集资产矩阵中直接切到“缺形态图”，定位计划或分镜已使用但还没有参考图的妆容/服装/状态形态。
- 对于已有父主图的资产，用户可以直接从矩阵触发第一个缺失形态的参考图生成；生成中的形态不会重复触发请求。

## 第一百九十五轮提交：跨集资产矩阵支持筛出重复身份风险

本轮把“同一角色/场景/物品被拆成多个项目资产”的风险前置到矩阵：

- `AssetContinuityPanel` 新增“重复身份”质量筛选，复用现有连续性报告中的 `duplicate_asset_name`、`duplicate_asset_alias`、`duplicate_library_entity_project_assets` 和 `cross_episode_duplicate_project_asset_candidate`。
- 矩阵头部新增重复身份风险数量摘要，数量继续跟随当前搜索、剧集和类型筛选范围。
- 状态列新增“重复身份”chip，hover 展示相关连续性报告消息，并显示关联候选数量，帮助用户判断是否需要进入详情抽屉合并或改名。
- 本轮只增强矩阵筛选、统计和状态可见性，不自动合并资产，不修改重复检测算法、合并算法、资产中心实体、剧集计划、分镜引用或项目持久化结构。

验收：

- 用户可以在跨集资产矩阵中直接切到“重复身份”，集中查看同名/别名冲突、同一身份多项目资产和跨集疑似重复资产。
- 重复身份风险会在矩阵行状态中明确显示，不再只能从连续性详情抽屉里逐条发现。

## 第一百九十六轮提交：跨集资产矩阵支持合并重复项目资产

本轮把“重复身份”视图从排查入口推进到可处理入口：

- `AssetContinuityPanel` 在重复身份风险行上新增“合并”按钮，只有连续性报告提供 `relatedAssetIds` 且候选目标仍是同类型、非子资产时才显示。
- 合并动作复用现有 `mergeProjectAssetInto`，与连续性详情抽屉和 Agent 工具保持同一条写入路径。
- 点击合并前保留确认弹窗，说明分镜引用和每集计划引用会迁移到目标资产，源资产会从项目资产中移除。
- 合并成功后沿用现有成功通知；本轮不新增自动合并策略，也不改变合并算法、重复检测算法、资产中心实体、剧集计划、分镜引用或项目持久化结构。

验收：

- 用户切到“重复身份”矩阵视图后，可以在有明确候选目标的行上直接触发项目资产合并，不必再打开连续性详情抽屉。
- 无有效同类型候选目标的重复风险行不会显示合并按钮，避免绕过既有合并边界。
