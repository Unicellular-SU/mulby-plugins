...
6. 顶栏「生成全剧」会按剧集顺序逐集执行资产、被分镜引用的形态参考图、关键帧、视频和合成；已成片、失败和暂缓剧集默认跳过，运行中可请求“暂停后续”并在当前集完成后停止，每集成片路径和制作回顾会回写到对应 Episode，供后续集生成时引用。
7. 需要重跑某集时，可在 Episode 切换器里重置当前集成片状态；尚未准备好的剧集可先标记为暂缓，之后恢复进入「生成全剧」队列，不必靠随手改分镜触发失效。
