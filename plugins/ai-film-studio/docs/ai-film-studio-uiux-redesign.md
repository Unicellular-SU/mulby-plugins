...
| 3 | 余下画布桥接：端口色（`PORT_COLORS`→`--afs-type-*`）/ NodeLibrary 分类点 / FlowCanvas 点阵·小地图（SVG `fill` 需 `getComputedStyle`） | ⏳ 待办 | 端口 Handle 是 DOM div，可直接内联 `var(--afs-type-*)`；React Flow Background/MiniMap 是 SVG 属性，需运行时读令牌值并随主题更新。 |
