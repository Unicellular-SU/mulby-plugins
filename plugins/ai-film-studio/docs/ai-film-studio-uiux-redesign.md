...
| 1 | 组件库 P0 · Select 迁移余下站点：**StudioSettings**（按 Agent 模型）/ **Inspector**（model/imageModel/provider + optgroup 片段插入器 + nodeDefs 参数 select） | ⏳ 进行中 | Inspector 片段插入器含 `<optgroup>`，届时为 `<Select>` 增加分组支持。每批 tsc+build 验证后提交。 |
