...
- 顶栏已有「生成全剧」入口，可按剧集顺序逐集执行资产、被分镜引用的形态参考图、关键帧、视频和合成；已成片、失败和暂缓剧集默认跳过，每集成片路径写回 `Episode.filmPath`，多集输出文件名包含集号和集标题。
...
- 多集批量生成队列：按集排队，失败集默认保持失败态并跳过，重置后重试；剧集可手动暂缓并从队列恢复；后续仍需补运行中的暂停。
