// ================= 共享并发池与重试（方案 4.2，遵守 D1/D4；7.1 收编进 manga-kit） =================
// 绘页阶段（App）与资产阶段（CharacterGenerator）共用；
// 中止机制沿用全局纪元 epoch（getEpoch 注入，插件侧绑定各自 scope 的 getAbortEpoch），
// 不引入 AbortController。
...
export async function asyncPool(
  ...
  const worker = async () => {
    while (true) {
      if (getEpoch() !== epoch) return; // 中止：不再取新任务
      const i = next++;
