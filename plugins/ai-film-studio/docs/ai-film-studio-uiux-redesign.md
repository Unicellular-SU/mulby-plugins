...
| 1 | 组件库 P0 · Select 迁移余下站点：**Studio**（artStyle/videoRatio/model-bar/storyboard/voice…）/ **Inspector**（model/imageModel/provider + optgroup 片段插入器 + nodeDefs 参数 select） | ⏳ 进行中 | 约 19 处原生 `<select>` 待换；Studio 拆 2 批；Inspector 片段插入器含 `<optgroup>`，届时为 `<Select>` 增加分组支持。每批 tsc+build 验证后提交。 |
