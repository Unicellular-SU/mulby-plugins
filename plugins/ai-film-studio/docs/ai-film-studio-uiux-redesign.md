...
| 1 | 组件库 P0 · Select 迁移 · **最后一站 Inspector（5 处，含 optgroup 片段插入器）** | ⏳ 进行中 | model/imageModel/provider override + nodeDefs 参数 select + 片段插入器（`<optgroup>` 分组）；将先为 `<Select>` 增加 optgroup 分组支持再迁移。完成后全仓原生 `<select>` 清零。 |
