...
| 1 | 组件库 P0 · Select 迁移余下站点：Inspector（含 optgroup 片段插入器）/ ProviderSettings / Studio（artStyle/videoRatio/…）/ ProjectHome / AssetsView | ⏳ 进行中 | 约 23 处原生 `<select>` 待逐批换；Inspector 片段插入器含 `<optgroup>`，届时为 `<Select>` 增加分组支持。每批 tsc+build 验证后提交。 |
