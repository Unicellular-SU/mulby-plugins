...
| 1 | 组件库 P0 · Select 迁移余下站点：Inspector / GlobalSettings / ProviderSettings / Studio（artStyle/videoRatio/…）/ ProjectHome / AssetsView | ⏳ 进行中 | 约 28 处原生 `<select>` 待逐批换成 `<Select>`（含 optgroup/并发非连续选项等特例）。每批 tsc+build 验证后提交。 |
