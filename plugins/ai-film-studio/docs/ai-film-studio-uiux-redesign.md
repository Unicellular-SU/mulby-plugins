...
| 1 | 组件库 P0 · Select 迁移余下站点：**Studio 批次2**（顶栏 artStyle/videoRatio、配音×2、分镜 shotSize/cameraMove、转场）/ **Inspector**（model/imageModel/provider + optgroup 片段插入器 + nodeDefs 参数 select） | ⏳ 进行中 | StudioEditor 余 7 处 + Inspector；Inspector 片段插入器含 `<optgroup>`，届时为 `<Select>` 增加分组支持。每批 tsc+build 验证后提交。 |
