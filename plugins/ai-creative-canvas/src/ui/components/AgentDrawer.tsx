...
import {
  AlertTriangle, Bot, Check, Circle, Clock3, Loader2, Pause,
  Play, RefreshCw, ShieldCheck, Sparkles, Square, Trash2, WandSparkles, X
} from 'lucide-react'
...
import {
  listEnabledWorkflowSkills, planScriptWorkflow, previewWorkflowSkills,
...
function PlanSummary({ plan }: { plan: GenerationPlan | null }) {
  ...
  return (
    <div className="rounded-lg border p-3" style={{ borderColor: 'var(--ace-border)' }}>
      ...
      {errors.length > 0 && <div className="mt-2 space-y-1">{errors.map((issue, index) => <div key={index} className="text-[10px] text-red-500">• {issue.message}</div>)}</div>}
...
export function AgentDrawer() {
  ...
  const [goal, setGoal] = useState('把剧本整理成可生成的分镜短片')
  ...
  const createInput = (): WorkflowPlanningInput | null => {
    ...
    return {
      sourceCard: entry.card, sourceBoardId: entry.board.id, project, goal,
  ...
  const startPlanning = async () => {
    ...
    if (!input) return toast('请先选择一张文本卡片', 'error')
    useWorkflowUi.getState().setPlanning(true)
    try {
      const run = await planScriptWorkflow(input)
  ...
  return (
    <aside data-interactive className={`fixed right-0 top-11 bottom-0 ${Z.panel} w-[460px] max-w-[92vw] ace-glass border-l flex flex-col ace-anim-scale`} style={{ borderColor: 'var(--ace-border)' }}>
      <header className="h-12 px-4 border-b flex items-center gap-2 shrink-0" style={{ borderColor: 'var(--ace-border)' }}>
        ...
        <span className="text-[10px] opacity-45">固定配方 · 可暂停恢复</span>
        <div className="flex-1" />
        {selectedRun && <button type="button" onClick={newPlan} className="text-[11px] text-indigo-500 hover:underline">新计划</button>}
      ...
      <div className="flex-1 overflow-auto ace-scroll p-4">
        {!selectedRun ? (
          <div className="space-y-4">
            <div className="rounded-xl bg-indigo-500/[0.08] p-3 text-xs leading-relaxed"><div className="font-semibold flex items-center gap-1.5"><WandSparkles size={14} />剧本转短片</div><div className="mt-1 opacity-65">AI 只生成创作规格与镜头草案；画布操作由插件白名单执行。首轮确认后生成静帧，确认静帧后再提交视频。</div></div>
            <label className="block text-[11px]"><span className="opacity-55">源文本卡片</span><Select className="mt-1" value={sourceCardId} onChange={setSourceCardId} placeholder="选择剧本或故事卡片" options={sourceCards.map(({ card, board }) => ({ value: card.id, label: card.title || '未命名文本', hint: board.name }))} /></label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <label className="text-[11px]"><span className="opacity-55">目标总时长（秒）</span><input className="ace-input mt-1" type="number" min="5" max="600" value={duration} onChange={(event) => setDuration(Math.max(5, Number(event.target.value) || 30))} /></label>
              <label className="text-[11px]"><span className="opacity-55">结尾要求（可选）</span><input className="ace-input mt-1" value={ending} onChange={(event) => setEnding(event.target.value)} placeholder="品牌露出 / 留白 / 反转" /></label>
            </div>
          ...
          <RunDetail run={selectedRun} plan={generationPlan} onNewPlan={newPlan} />
        )}
      </div>
    </aside>
  )
...
function RunDetail({ run, plan, onNewPlan }: { run: WorkflowRun; plan: GenerationPlan | null; onNewPlan: () => void }) {
  ...
  const remove = async () => {
    const ok = await confirmDialog({ title: '删除工作流记录', message: '只删除计划与日志，已经生成的画布卡片会保留。', confirmLabel: '删除', cancelLabel: '取消', danger: true })
    if (!ok) return
  ...
  return <div className="space-y-4">
    <div>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5 text-[10px]"><span className="px-2 py-1 rounded bg-black/5 dark:bg-white/10">{run.brief.audience}</span><span className="px-2 py-1 rounded bg-black/5 dark:bg-white/10">{run.brief.aspect}</span><span className="px-2 py-1 rounded bg-black/5 dark:bg-white/10">约 {run.brief.totalDuration}s</span><span className="px-2 py-1 rounded bg-black/5 dark:bg-white/10">{run.brief.shots.length} 镜</span></div>
    </div>
    {stale && <div className="rounded-lg bg-red-500/10 text-red-600 dark:text-red-300 p-3 text-xs"><div className="font-semibold">源文本已修改</div><div className="mt-1 opacity-70">当前计划已被冻结，防止按旧剧本继续生成。请新建计划。</div></div>}
    ...
    <div className="grid grid-cols-2 gap-2">
      ...
      <button type="button" onClick={() => void remove()} className="h-8 rounded-md text-red-500 hover:bg-red-500/10 text-[11px] flex items-center justify-center gap-1"><Trash2 size={12} />删除记录</button>
    </div>
