...
import { useAssetStore, type ElementRef } from '../store/assetStore'
...
function notifyAdded(ok: boolean) {
  window.mulby?.notification?.show(ok ? '已加入项目素材（在「资产」分组查看 / 编辑）' : '没有打开的项目', ok ? 'success' : 'warning')
}
...
function AssetInsertPanel() {
  ...
  const pid = useProjectStore((s) => s.doc?.meta.id)
  ...
  // 把库元素 / 图片素材绑成「项目资产」（含参考图）；与「点击插名称」并存，互不影响
  const addElement = async (el: ElementRef) => {
    if (!pid) return notifyAdded(false)
    const id = await importElementToProject(pid, el)
    if (id) notifyAdded(true)
  }
  const addAsset = async (a: AssetRecord) => {
    if (!pid) return notifyAdded(false)
    const id = await importImageToProject(pid, a, 'role') // 默认角色，进项目后可在「资产」改类型
    if (id) notifyAdded(true)
  }
  ...
  return (
    <div className="afs-dockpanel">
      ...
      <div className="afs-dockpanel__hint">点击插入名称到聚焦输入框；右上「＋」把素材 / 角色加入本项目资产（带参考图，可直接生成关键帧）</div>
