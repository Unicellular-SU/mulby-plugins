    ...
    >
      <div className="p-3">
        ...
        <div className="space-y-1.5 text-xs font-mono text-slate-300">
             ...
             <div className="flex justify-between items-center">
                <span className="text-slate-500">Images Generated:</span>
        ...
        {isExpanded && (
            <div className="mt-3 pt-2 border-t border-slate-700 animate-fade-in space-y-4">
                ...
                {/* Cost Breakdown */}
                <div>
                  <p className="text-[10px] text-slate-500 mb-1 uppercase tracking-wider font-bold">Cost Breakdown</p>
                  <div className="space-y-1 text-[10px] text-slate-400">
                    <div className="flex justify-between">
                      <span>Gemini 3.0 Pro (Text):</span>
                      <span className="text-slate-200">${usage.breakdown.gemini3ProCost.toFixed(4)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Gemini 3.0 Pro (Images):</span>
                      <span className="text-slate-200">${usage.breakdown.gemini3ProImageCost.toFixed(4)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Flash 2.5 (Char Refs):</span>
                      <span className="text-slate-200">${usage.breakdown.gemini25FlashImageCost.toFixed(4)}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Detailed Logs */}
                <div>
                    <p className="text-[10px] text-slate-500 mb-2 uppercase tracking-wider font-bold">Recent Activity</p>
                            ...
                            usage.history.slice().reverse().map((h, i) => (
                                <div key={i} className="flex flex-col border-b border-slate-800/50 pb-1 last:border-0">
                                    <div className="flex justify-between items-baseline">
                                        ...
                                        <span className="text-[10px] text-green-400 font-mono">+${h.cost.toFixed(4)}</span>
                                    </div>
                                    <div className="flex justify-between text-[9px] text-slate-500 mt-0.5">
                                        <span>
                                            {h.stat.modelType.replace('GEMINI_', '').replace('IMAGE', 'IMG')}
                                        </span>
