...
- Episode 切换器已有跨集承接入口，会主动汇总最近制作回顾，以及当前集资产/形态在其他剧集中的出现记录；还会给出补参考图、标记形态适用本集、新建并应用本集形态等建议。Agent 也可用 `get_episode_handoff` 读取同一套线索和建议。
...
- 将跨集承接建议继续深化为自动化修复：批量补齐参考图、基于上一集状态生成新妆容提示词、自动回写推荐变体。
...
16. 下一步：把承接建议深化为批量自动修复和基于上一集状态的提示词生成。
