...
- **跨集承接线索**：Episode 切换器可查看最近制作回顾和当前集复用过的资产/形态；Agent 也可通过 `get_episode_handoff` 读取同一套线索。
...
6. Episode 切换器旁的承接入口会汇总最近制作回顾，以及当前集角色/场景/道具形态在其他集里的出现记录；处理续集、换装或妆容变化前先看这里。
...
- [ ] Episode 切换器旁的承接入口能显示最近制作回顾和当前集复用资产/形态；Agent 工具能读取 `get_episode_handoff`。
...
| M11 | 多集生产队列与承接线索（生成全剧、暂缓/暂停、制作回顾、handoff 工具） | ✅ |
