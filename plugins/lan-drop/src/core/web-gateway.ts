...
export class WebGateway {
  ...
  offerToDevice(
    ...
    this.sendOffers(s)
    void notify(`已向 ${s.name} 推送 ${files.length} 个文件，等待手机下载`, 'info')
    return { ok: true, ids }
  }

  private sendOffers(s: WebSession): void {
    if (!s.sse) return
    const list = [...s.outbox.values()].map((o) => ({
      id: o.id,
      name: o.name,
      relPath: o.relPath,
      size: o.size,
  ...
  private handleOutbox(req: http.IncomingMessage, res: http.ServerResponse): void {
    ...
    this.sendJson(res, 200, { ok: true, offers: list })
  }
  ...
  private async handleDownload(
    ...
    let size = offer.size
    try {
      const st = fs.statSync(offer.filePath)
    ...
    const rs = fs.createReadStream(offer.filePath)
    let sent = 0
    let lastBytes = 0
    let lastTime = Date.now()
    let lastEmit = 0
