...
- Episode 切换器已有跨集承接入口，会主动汇总最近制作回顾，以及当前集资产/形态在其他剧集中的出现记录；Agent 也可用 `get_episode_handoff` 读取同一套线索。
- Episode 切换器已有重置当前集成片状态入口，可清理 `filmPath/filmError/producedAt/productionRecap`，让已成片或失败剧集重新进入「生成全剧」队列；也可把未准备好的剧集标记为暂缓，之后恢复入队。

后续重点不再是“能不能表达多集”，而是全季导出、跨集记忆召回的自动应用和更细的自动化修复。
...
- 支持“一集一包导出”和“全季导出”。
- 将跨集承接线索进一步用于自动建议：沿用上一集形态、创建新妆容变体、标记变体适用集或提示缺失参考图。
...
11. 下一步：把承接线索升级为自动化建议，辅助创建新妆容变体、补齐参考图、标记变体适用剧集，并补全“一集一包/全季导出”。
