...
6. 顶栏「生成全剧」会按剧集顺序逐集执行资产、被分镜引用的形态参考图、关键帧、视频和合成；已成片剧集默认跳过，每集成片路径和制作回顾会回写到对应 Episode，供后续集生成时引用。
7. 需要重跑某集时，可在 Episode 切换器里重置当前集成片状态；该集会重新进入「生成全剧」队列，不必靠随手改分镜触发失效。
...
- [ ] 顶栏「生成全剧」能逐集生成资产、分镜实际引用的形态参考图并合成，已成片剧集会跳过；多集输出文件名包含集号和集标题，切回对应剧集能看到「已成片」状态。
