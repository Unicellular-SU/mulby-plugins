...
interface UseImageQueueDeps {
  ...
  /** Phase 2 水印（features.watermark）：返回该页生效的水印设置（本页覆盖 || 全局）；
   *  开关关闭时不传此回调，生成链路与 Phase 1 完全一致 */
  getEffectiveWatermark?: (page: ComicPageData) => WatermarkSettings | undefined;
}
...
export const useImageQueue = ({
  ...
  const S = getTheme().strings; // 主题在插件生命周期内不可变，hook 内捕获一次即可
  ...
  const triggerImageGeneration = async (page: ComicPageData, ratio: string, references?: ImageReferenceInput) => {
    ...
    try {
        ...
        // enabled=false 时 applyWatermark 原样直通。开关关闭时无 getEffectiveWatermark，行为与 Phase 1 一致
        const wmSettings = getEffectiveWatermark?.(page);
        const imageData = wmSettings
            ? await applyWatermark(base64Image, wmSettings, getTheme().watermark?.fallbackText)
            : base64Image;
        if (isStale(runEpoch)) return;

        // 单页成功即增量落盘（方案 3.1 步骤 3）：fire-and-forget，失败不打断生成流程
        void putImageAttachment(attIdForPage(page.page_number), imageData);

        setPages(prev => prev.map(p =>
            p.page_number === page.page_number
            ? { ...p, imageData, ...(wmSettings ? { rawImageData: base64Image } : {}), isGenerating: false, error: undefined, progress: undefined }
            : p
        ));
    } catch (error: any) {
