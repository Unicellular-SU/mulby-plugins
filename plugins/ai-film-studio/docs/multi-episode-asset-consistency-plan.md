...
- 顶栏已有「导出全季」入口，会把已成片剧集复制到用户选择的目录，并写入 `manifest.json` 记录集号、标题、源路径、导出路径、素材引用表和缺失项报告。
- Episode 切换器已有「导出当前集」入口，会把当前集成片复制到用户选择的目录，并写入 `episode.json` 记录本集剧本、分镜、制作回顾、导出路径、素材引用表和缺失项报告。
- Episode 切换器已有重置当前集成片状态入口，可清理 `filmPath/filmError/producedAt/productionRecap`，让已成片或失败剧集重新进入「生成全剧」队列；也可把未准备好的剧集标记为暂缓，之后恢复入队。

后续重点不再是“能不能表达多集”，而是字幕交付补全、跨集记忆召回的自动应用和更深的自动化修复。
...
- 继续增强交付清单，例如附带字幕，并维护已落地的素材引用表和缺失项报告。
...
15. 下一步：补齐字幕交付，并把承接建议深化为批量自动修复和基于上一集状态的提示词生成。
