    ...
    <div className="flex items-center gap-0.5 max-w-[92vw] overflow-x-auto ace-noscroll">{/* 单行横向滚动，不再 wrap 成多排挤压卡片 */}
      {isImg && (
        <>
          ...
          {isPano && <IconBtn icon={GitMerge} title="修复接缝" onClick={() => void repairEquirectSeam(card.id)} />}
          {isPano && <IconBtn icon={ArrowUpDown} title="天/地修复" onClick={() => void repairEquirectPoles(card.id)} />}
          {/* 裁剪/扩图/宫格/抠像/局部编辑会破坏等距柱状 2:1 投影，全景卡不提供 */}
          {!isPano && <IconBtn icon={Crop} title="裁剪" onClick={() => setCropping(true)} />}
          {!isPano && <IconBtn icon={Brush} title="局部编辑" onClick={() => useUi.getState().setMaskCardId(card.id)} />}
      ...
      {isVid && (
        <>
          ...
          <IconBtn icon={Wand2} title="绿幕抠像" onClick={() => runVideoTool(card.id, 'chromakey')} />
          <IconBtn icon={Download} title="下载" onClick={() => void download()} />
        </>
      )}
      {cropping && (
