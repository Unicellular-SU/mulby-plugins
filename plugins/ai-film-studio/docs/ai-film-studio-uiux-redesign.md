...
| 1 | 组件库 P0 · 余下：自定义 Select+Combobox（headless 弹层）/ Switch / Segmented / Tabs / Field 容器 | ⏳ 待办 | 自定义 Select/Combobox/Switch 需引入 headless 行为库（Radix / React Aria）或手写 listbox，属较重改造，单列。Segmented/Tabs 可先做纯 CSS 版。 |
| — | 残留琥珀清理跟踪 | ⏳ 待办 | `styles.css` 仍有 `#f59e0b`：孤儿 `.afs-toolbar__logo`（应删）、advbanner 警告条、studio 模型缺失/错误态——警告类应改 `--afs-warning` 令牌，随设置/studio/清理阶段处理。 |
