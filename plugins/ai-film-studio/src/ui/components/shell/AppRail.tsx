import { Clapperboard, LayoutGrid, Images, Server, HardDrive, SlidersHorizontal, Sun, Moon, type LucideIcon } from 'lucide-react'
import { useUiStore } from '../../store/uiStore'

export type AppView = 'studio' | 'home' | 'editor' | 'assets' | 'prompts' | 'models' | 'storage' | 'advanced'

// 一级导航：项目 / 素材 + 设置子项（模型 / 存储 / 高级）。画布与工作台不作一级入口——只能从「项目」页进入。
const ITEMS: { view: AppView; icon: LucideIcon; label: string }[] = [
  { view: 'home', icon: LayoutGrid, label: '项目' }, // 统一项目主页：画布工程 + 工作流项目
  { view: 'assets', icon: Images, label: '素材' }, // 素材库：素材 / 角色场景库 / 提示词（导入导出本地素材）
...
/** 左侧一级界面导航栏（rail）：始终可见——项目 / 素材 / 模型 / 存储 / 高级；底部主题切换。 */
