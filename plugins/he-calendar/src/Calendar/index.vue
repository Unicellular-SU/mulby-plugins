...
// 统一的存储工具函数，优先使用 Mulby storage，降级到 localStorage
const getLocalStorageItem = (key, defaultValue) => {
  return localStorage.getItem(key) || defaultValue;
};

const getStorageItem = async (key, defaultValue) => {
  if (window.mulby && window.mulby.storage) {
    const value = await window.mulby.storage.get(key);
...
const setStorageItem = async (key, value) => {
  if (window.mulby && window.mulby.storage) {
    await window.mulby.storage.set(key, value);
  }
  localStorage.setItem(key, value);
};

const colorMode = ref(getLocalStorageItem('calendar-color-mode', 'auto')); // light, dark, auto
const weekStartDay = ref(parseInt(getLocalStorageItem('calendar-week-start', '0')));
const previewTheme = ref(null);
const yearPickerOffset = ref(0); // 年份选择器的偏移量

// 节假日显示开关（四大分类，法定假日始终显示）
const showSolarFestivals = ref(getLocalStorageItem('calendar-show-solar-festivals', 'true') === 'true'); // 公历现代节日
const showLunarFestivals = ref(getLocalStorageItem('calendar-show-lunar-festivals', 'true') === 'true'); // 农历传统节日
const showInternationalFestivals = ref(getLocalStorageItem('calendar-show-international-festivals', 'true') === 'true'); // 国际节日
const showSolarTerms = ref(getLocalStorageItem('calendar-show-solar-terms', 'true') === 'true'); // 二十四节气
...
const switchTheme = async (themeId) => {
  ...
  await setStorageItem('calendar-theme', themeId);
  applyTheme();
...
const switchColorMode = async (mode) => {
  ...
  await setStorageItem('calendar-color-mode', mode);
  applyTheme();
};
...
const setWeekStartDay = async (day) => {
  ...
  await setStorageItem('calendar-week-start', day.toString());
};

// 节假日显示开关控制
const toggleSolarFestivals = async () => {
  showSolarFestivals.value = !showSolarFestivals.value;
  await setStorageItem('calendar-show-solar-festivals', showSolarFestivals.value.toString());
};

const toggleLunarFestivals = async () => {
  showLunarFestivals.value = !showLunarFestivals.value;
  await setStorageItem('calendar-show-lunar-festivals', showLunarFestivals.value.toString());
};

const toggleInternationalFestivals = async () => {
  showInternationalFestivals.value = !showInternationalFestivals.value;
  await setStorageItem('calendar-show-international-festivals', showInternationalFestivals.value.toString());
};
...
const toggleSolarTerms = async () => {
  ...
  await setStorageItem('calendar-show-solar-terms', showSolarTerms.value.toString());
};
...
const openExternalLink = (url) => {
  if (window.mulby && window.mulby.shell) {
    // Mulby 环境下使用 shell.openExternal API
    window.mulby.shell.openExternal(url);
  } else {
...
onMounted(async () => {
  // Load settings from Mulby storage (async), fallback to localStorage
  const savedTheme = await getStorageItem('calendar-theme', 'auto');
  currentTheme.value = savedTheme;
  colorMode.value = await getStorageItem('calendar-color-mode', 'auto');
  weekStartDay.value = parseInt(await getStorageItem('calendar-week-start', '0'));
  showSolarFestivals.value = (await getStorageItem('calendar-show-solar-festivals', 'true')) === 'true';
  showLunarFestivals.value = (await getStorageItem('calendar-show-lunar-festivals', 'true')) === 'true';
  showInternationalFestivals.value = (await getStorageItem('calendar-show-international-festivals', 'true')) === 'true';
  showSolarTerms.value = (await getStorageItem('calendar-show-solar-terms', 'true')) === 'true';
  ...
  // Register Mulby plugin init listener
  if (window.mulby) {
    window.mulby.onPluginInit((data) => {
      if (data.featureCode) {
        // Handle feature-specific init if needed
      }
    });
  }
