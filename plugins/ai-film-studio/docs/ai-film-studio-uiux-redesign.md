...
| 3 | 余下画布桥接：FlowCanvas 点阵/遮罩/小地图（SVG `fill`） | ⏳ 待办 | `dotColor`/`maskColor` 仍是按主题分支的 JS 字面量（可对齐到 `--afs-grid-dot`/`--afs-scrim` 值），`miniMapColor` 用 `CATEGORY_META[..].color`（SVG 属性不解析 `var()`，需 `getComputedStyle` 读 `--afs-cat-*` 并随主题更新）。 |
