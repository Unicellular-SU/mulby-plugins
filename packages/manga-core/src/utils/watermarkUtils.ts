import { WatermarkSettings, WatermarkType } from '../engine-types';

// ================= Canvas 水印（Phase 2：原样搬自 horror-manga/utils/watermarkUtils.ts） =================
// 唯一通用化改动：settings.text 为空时的兜底文字由调用方按主题注入（fallbackText），
// 不再硬编码题材品牌名。enabled=false 时原样返回（零开销直通）。
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = async () => {
      ...
      if (settings.type === WatermarkType.TEXT_TILED || settings.type === WatermarkType.TEXT_CORNER) {
        // --- TEXT WATERMARK ---
        const text = settings.text || fallbackText;
