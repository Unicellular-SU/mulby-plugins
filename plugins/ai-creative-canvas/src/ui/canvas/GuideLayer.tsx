  ...
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ overflow: 'visible' }}>
      {guides.vx.map((x, i) => {
        ...
        return <line key={'v' + i} x1={sx} y1={0} x2={sx} y2={ss.h} stroke="#ec4899" strokeWidth={1} strokeDasharray="4 3" />
      })}
      {guides.hy.map((y, i) => {
        const sy = worldToScreen(0, y, vp).y
        return <line key={'h' + i} x1={0} y1={sy} x2={ss.w} y2={sy} stroke="#ec4899" strokeWidth={1} strokeDasharray="4 3" />
      })}
    </svg>
  )
}
