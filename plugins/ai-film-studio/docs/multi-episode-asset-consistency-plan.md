...
- 资产页已有跨集资产矩阵，分镜页已有一致性详情抽屉；连续性报告会标记同类型角色/场景/道具重名、章节分配、变体适用范围和缺失参考图，并提供生成缺失参考图、标记变体适用本集、重新均分章节等快速修复。
...
- 系列一致性检查：已支持同类型资产重名检测，后续继续扩展角色别名、资产复用、变体覆盖和场景复用。
...
18. 下一步：继续扩展系列一致性质量门，把角色别名、资产复用、变体覆盖和跨集状态变化纳入更自动的检查与 Agent 生成约束。
